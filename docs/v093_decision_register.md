...
| D093-011 | Ordinary crack-width acceptance is optional. | Sector always displays the calculated crack width when available. With no user criterion it reports `CALCULATED - ACCEPTANCE NOT ASSESSED`. When a positive finite user criterion is supplied, Sector publishes the limit, ratio and the controlled result `WITHIN USER-SPECIFIED LIMIT` or `EXCEEDS USER-SPECIFIED LIMIT`. It does not infer an exposure limit or global compliance, and it does not reuse demand/resistance `PASS`/`FAIL` terminology. | Solver, schema, UI, cache-signature, report and manual tests. | PR-06 |
...
| D093-013 | Confinement is deferred beyond 0.93. | No DS/EN 1992-1-1:2023 confinement input, solver equation or enhancement, result claim, or placeholder selector is added in this programme. Relevant manual and report scopes explicitly disclose that confinement enhancement is not included or assessed. | Absence assertions, limitation-disclosure tests and scope review. | PR-02 and PR-09 |
...
| D093-015 | Keep current- and next-generation concrete design options only where Sector implements a verified calculation. | First-generation bridge references remain DS/EN 1992-2:2005 + AC:2008 only beside retained calculations that use them. DK NA:2015 remains project context in v0.93 and is not an exposed option unless a retained solver declares an exact verified NA-specific capability. The second generation is DS/EN 1992-1-1:2023, whose normative Annex K defines bridge scope; that fact is standards context, not a Sector implementation claim. No fictitious `EN 1992-2:2023` option is created. The 2023 choice is labelled a published project-adoption option with no Danish NA applied. | Registry, citation, selector, report and absence tests. | PR-02 and PR-06 |
...
| D093-026 | The decision register is also delivered as `docs/sector_v093_decision_register.xlsx`. | The workbook mirrors these IDs, includes programme ownership and acceptance columns, uses filters/frozen headers, records the programme baseline and LF-normalized canonical Markdown SHA-256, and is pinned by the accepted Git revision. Environment paths, tenant labels and active OOXML parts are prohibited. | Workbook formula/error/metadata inspection plus rendered review of every sheet. | PR-01 follow-up and PR-09 |
| D093-027 | Sector never issues a global code-compliance conclusion. | `PASS`/`FAIL` remains reserved for an implemented demand-versus-resistance equation. A user-specified output-limit comparison uses `WITHIN USER-SPECIFIED LIMIT` or `EXCEEDS USER-SPECIFIED LIMIT` and publishes the criterion source. Output-only and omitted-criterion states use `CALCULATED`, `NOT ASSESSED`, `NOT APPLICABLE` or `NOT REQUESTED` as appropriate. | Status-taxonomy and absence tests across UI/report/manual. | PR-06, PR-07B and PR-09 |
...
| First-generation concrete bridges | DS/EN 1992-2:2005 + AC:2008; DK NA:2015 is project context only in v0.93 | Current local bridge family, subject to project and owner requirements | Remove component-mapped checks. The retained corrected concrete-fatigue Formula 6.106 route is base + AC only. Do not expose a DK NA selector without an exact capability. No complete bridge-design claim. |
| Second-generation concrete and bridges | DS/EN 1992-1-1:2023, including normative Annex K as standard scope, not a Sector implementation claim | Published reference option; project adoption required; no Danish NA applied | Retain or add only verified non-component equations. Disclose each Eurocode recommended value used for a Nationally Determined Parameter and that no Danish national choice is applied. No confinement enhancement in 0.93; disclose that limitation. |
