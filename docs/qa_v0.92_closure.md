...
| F-005 | PR-04 | Implemented on branch; exact-head external gates pending | A distinct `DS/EN 1992-2:2005 + AC:2008` method records inherited, overridden, added and not-assessed checks; calculates brittle Method b, separate box-wall torsion, bridge SLS stress/crack routing, separate web/flange minimum reinforcement and bridge concrete fatigue; mandatory unsupported states block overall PASS. | Controlled local standard matrix plus independent `tools/pr04_bridge_oracle.py` examples for brittle reinforcement, common-angle wall torsion, corrected concrete Miner life, bridge SLS limits and separate web/flange minimum. | 387 bridge/SLS/fatigue core checks; 214 project/presentation/manual checks with one skip; 142 report checks; 3 rendered-artifact checks; 19 version/package/provenance controls; 10 focused Streamlit checks. | Pending exact-head Codex Review after commit and CI. | Pending original reviewer. | Pending | Methods a/c, added bridge web/interface interaction, shear/torsion fatigue, deflection and opened segmental-joint checks remain explicit blocking `NOT ASSESSED` when applicable. |
...
| F-010 | PR-04 | Implemented on branch; exact-head external gates pending | The corrected DS/EN 1992-2 Expression (6.106) concrete Miner route is available under the bridge methodology; ordinary EN 1992-1-1 use requires explicit approved project adoption and source, otherwise it blocks. Result, persistence, UI and report surfaces retain the selected applicability and provenance. | Controlled local DS/EN 1992-2:2005 clause 6.8.7(101) with AC:2008 correction; independent `tools/pr04_bridge_oracle.py` life/damage examples. | Included in the 387 core, 214 boundary/manual, 142 report, 3 rendered-artifact and 10 focused Streamlit checks recorded for PR-04. | Pending exact-head Codex Review after commit and CI. | Pending original reviewer. | Pending | Project-basis adoption outside the bridge method remains an explicit engineering-authority decision with a recorded approval source. |
