...
- Initial implementation head
  ...
  generated multi-case PDF are covered by red/green regressions. The bounded
  corrective gate passes 171 checks, including the complete PR-06 interaction
  suite, affected report/publication checks and ASCII/version controls; the
  standalone independent oracle remains green.
- Exact corrective SHA, current-head Codex rereview, the single completed
  ...
  remain pending. The automatically started initial-head CI run is superseded
  and is not counted as the required exact-head gate.
