# PR-14C2R3 acceptance: raw source snapshot and package identity

## Bounded objective

This clean reslice implements F-021 C2 only. It derives package time and
...
PR #370 and PR #371, together with their branches, commits, review history,
Actions runs, and artifacts, are preserved negative evidence. Neither closed
candidate is in this branch's ancestry. PR #371 used its single review
amendment and was then closed unchanged when the complete Actions suite exposed
stale retained product-identity expectations.
...
## Retained-contract reslice correction

The preserved PR #371 Actions run completed the C2 implementation paths,
report render, manual render, and evidence upload, but the full suite found one
localized retained-contract defect class: seven tests still expected the
removed Git/worktree/GITHUB fallback and wall-clock manifest construction. This
clean reslice replaces those retired expectations with direct oracles for the
complete canonical environment seal, missing-field rejection despite a
`GITHUB_SHA` fallback, noncanonical/mismatched field rejection, source-epoch
manifest construction, and the absence of Git/checkout identity helpers.
...
- retained product-identity red oracle: 7 expected failures and 4 passes;
- corrected retained product-identity contract: 13 passed;
- complete directly affected exporter/build/release/unsigned-QA/packaging and
  retained Windows identity boundary: 125 passed, 1 inherited symlink skip;
- coverage/dependency/Ruff/mypy/legal/reproducibility/ASCII/version and retained
  identity guards: 358 passed;
...
- lock preflight, enforced Ruff policy, enforced strict mypy policy, pyflakes,
  py_compile, YAML parsing, and diff checks: clean;
