...
| D095-021 | Add no unapproved feature or global compliance scope. | Sector 0.95 adds no selectable design basis, geometry or certification/approval claim. Only D095-023 additions are authorized; schema changes only under D095-024. | Diff, status-vocabulary and schema guards. | All |
...
| D095-024 | Use two independent user-owned ordinary crack-width criteria. | Long-term and short-term widths compare only with their matching positive finite criterion. Exact zero disables only that comparison while retaining the calculated width. Schema-25 positive single criteria copy to both schema-26 values; blank migrates to two zeros. The optional DK heightened route owns a separate positive formula operand. | Exact zero/positive/malformed, independent-duration, migration and formula-operand matrices. | PR-A04 |
| D095-025 | Freeze each addition's detailed acceptance matrix in its owning PR before code. | PR-A00 freezes scope, dependencies and ownership only. In particular PR-A05 owns the exact eligible fatigue class-to-threshold map and below/equal/above outcomes; PR-A06 owns the complete emitted status precedence, deterministic numeric/status ties and selected provenance. Neither deferred matrix is implementation evidence until its bounded PR freezes it. | Per-slice matrix, exclusions, hostile controls and exact-head review. | PR-A01 through PR-A10 |
