...
def _exact(actual: Any, expected: Any, label: str) -> None:
    """Exact retained Python value comparison, including container types."""
...
def _shape(actual: Any, expected: Any, label: str) -> None:
    """Fence excluded concrete siblings without owning their values."""
...
def _fingerprint(value: Any) -> str:
    data = json.dumps(
        _identity(value), ensure_ascii=True, separators=(",", ":")
    ).encode("ascii")
    ...
    def factors(self, inp: Mapping[str, Any]):
        ...
        for step_id, key, title in (
            ...
            output.append(self.add(
                step_id, title, ONE, ROLE_USER_INPUT, INPUT, value=value
            ))
        return tuple(output)
...
def _yield(stress: float, prop: ReinforcementFatigueProperties, gamma_s: float):
    characteristic = (
        prop.fytk_mpa
        if stress >= 0.0 or prop.fyck_mpa is None
        else prop.fyck_mpa
    )
    ...
    *,
    ...
    for node in member.nodes:
        ...
        if result.state == RESULT_FINITE:
            shown = f"{result.value:.17g} {node.unit.symbol}"
...
def _bin_nodes(
    ...
    # This is the independent assessment-to-Elastic join. The assessment is not
    # permitted to serve as its own stress oracle.
    for actual, wanted, label in (
        ...
        (assessment.stress_total_elastic_mpa, elastic_total,
         "perfect-bond stress"),
    ):
        _same_number(actual, wanted, f"{prefix} {label}")
    if type(assessment.converged) is not bool or (
        assessment.converged != state.converged
    ):
    ...
    cycles = graph.add(
        f"{prefix}-cycles", "Applied cycles", CYCLES, ROLE_COMPUTED,
        ELASTIC, normal, value=state.cycles,
    )
    long_id = graph.add(
        f"{prefix}-elastic-long-stress", "Matched Elastic long stress", MPA,
        ROLE_COMPUTED, ELASTIC, normal, value=long_stress,
    )
    total_id = graph.add(
        f"{prefix}-elastic-fatigue-total-stress",
        "Matched Elastic fatigue total stress", MPA, ROLE_COMPUTED, ELASTIC,
        normal, value=fatigue_total,
    )
    design_id = graph.add(
        f"{prefix}-elastic-design-total-stress",
        "Matched Elastic action-factored total stress", MPA, ROLE_COMPUTED,
        ELASTIC, normal, gamma_ff_id, value=design_total,
    )
    perfect_id = graph.add(
        f"{prefix}-elastic-perfect-bond-total-stress",
        "Matched Elastic perfect-bond total stress", MPA, ROLE_COMPUTED,
        ELASTIC, normal, value=elastic_total,
    )
    convergence = graph.add(
        f"{prefix}-elastic-converged", "Matched Elastic convergence", ONE,
        ROLE_COMPUTED, ELASTIC, normal, value=float(state.converged),
    )
    bond_identity = graph.add(
        f"{prefix}-bond-{_fingerprint(state.bond_method)}",
        "Exact matched Elastic bond-method identity", ONE, ROLE_COMPUTED,
        ELASTIC, normal,
    )
    ...
    for actual, wanted, label in (
        ...
        (assessment.stress_range_elastic_mpa, elastic_range,
         "perfect-bond range"),
    ...
    elastic_range_id = graph.add(
        ...
        ROLE_COMPUTED, ELASTIC, perfect_id, long_id, value=elastic_range,
    )
    bond_factor_id = graph.add(
        f"{prefix}-bond-adjustment", "Bond adjustment", ONE,
        ROLE_COMPUTED, ELASTIC, range_id, elastic_range_id, bond_identity,
        value=bond_factor,
    )
    ...
    reference = graph.add(
        ...
        value=prop.delta_sigma_rsk_mpa,
    )
    ...
    governing_stress = graph.add(
        ...
        ROLE_COMPUTED, yield_source, long_id, design_id, value=gov_stress,
    )
    limit_id = graph.add(
        f"{prefix}-yield-limit", "Design proof-stress limit", MPA,
        ROLE_COMPUTED, yield_source, governing_stress, gamma_s_id, value=limit,
    )
    yield_utilisation = graph.add(
        f"{prefix}-yield-utilisation", "Yield utilisation", ONE,
        ROLE_COMPUTED, yield_source, governing_stress, limit_id, value=util,
    )
...
def _assessment_member(
    ...
    edition = _analysis().prepare(inp).edition
    sn_source, yield_source = code_sources(edition)
    ...
    for bin_index, (item, state) in enumerate(zip(
        assessment.bins, spectrum.bins
    )):
        ...
        damage, yield_util, convergence = _bin_nodes(
            ...
            gamma_s_id, gamma_ff_id, float(inp["fatigue_gamma_s"]),
            sn_source, yield_source,
        )
    ...
    damage_identity = graph.add(
        ...
        "Exact governing damage-bin identity", ONE, ROLE_COMPUTED,
        VERDICT, *damages,
    )
    yield_max = graph.add(
        "element-yield-utilisation", "Element yield utilisation", ONE,
        ROLE_COMPUTED, yield_source, *yields, value=max_yield,
    )
    yield_identity = graph.add(
        f"governing-yield-bin-{trace_identity_token(yield_row.bin_name)}",
        "Exact governing yield-bin identity", ONE, ROLE_COMPUTED,
        VERDICT, *yields,
    )
...
def _aggregate_member(inp, spectra, context):
    ...
    for spectrum_index, spectrum in enumerate(spectra):
        ...
        for element_index, item in enumerate(spectrum.reinforcement):
            step_id = graph.add(
                ...
                ONE, ROLE_COMPUTED, VERDICT, normal, value=item.utilisation,
            )
    ...
    governing = graph.add(
        ...
        "Governing reinforcement utilisation", ONE, ROLE_COMPUTED,
        VERDICT, *assessment_ids, value=utilisation,
    )
    status = graph.add(
    ...
    graph.add(
        ...
        normal, gamma_s, gamma_ff, gamma_c, *assessment_ids, *governing_ids,
        governing, status, value=utilisation,
    )
    member = MemberSpec(
...
def _build_expected(inp, out, input_sha256, result_sha256, context):
    ...
    if "valid" in candidate:
        ...
        analysis = _analysis()
        # A concrete-only candidate is replayed and completely shape-validated
        # before CT-010a returns inapplicable. An empty/truncated mapping can never
        # bypass the retained application boundary.
