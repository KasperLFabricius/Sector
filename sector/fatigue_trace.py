...
from .fatigue_trace_contract import (
    ...
    IdentityLeaf, aggregate_steps, assessment_prefix, assessment_steps,
    bin_prefix, expected_registry, invalid_plan, invalid_steps, success_plan,
)
...
def _compare_bin(actual, expected, label):
    ...
    for field in (
        ...
        _compare_exact(getattr(actual, field), getattr(expected, field),
                       f"{label}.{field}")
    for field in (
        "concrete_compression_long_mpa", "concrete_compression_total_mpa",
        "concrete_compression_design_total_mpa",
    ):
        _compare_shape(getattr(actual, field), getattr(expected, field),
                       f"{label}.{field}")
...
def _flatten(value, path, leaves):
    ...
    if isinstance(value, np.ndarray):
        ...
        leaves.append(IdentityLeaf(
            f"{path}-ndarray-{dtype}", path, float(value.size)
        ))
        ...
        leaves.append(IdentityLeaf(f"{path}-{kind}", path, float(len(value))))
        for index, item in enumerate(value):
            _flatten(item, f"{path}-i{index:04d}", leaves)
    elif isinstance(value, Mapping):
        mapping_type = "-".join(_type_identity(value)).encode("utf-8").hex()
        leaves.append(IdentityLeaf(
            f"{path}-mapping-{mapping_type}", path, float(len(value))
        ))
        for index, (key, item) in enumerate(value.items()):
            if type(key) is not str:
                raise TraceValidationError("retained identity keys must be strings")
            key_token = key.encode("utf-8").hex()
            _flatten(item, f"{path}-k{index:04d}-u{key_token}", leaves)
        ...
        leaves.append(IdentityLeaf(
            f"{path}-number-{type(value).__name__}", path, number
        ))
    else:
        raise TraceValidationError(f"unsupported retained identity at {path}")


def _raw_controls(inp):
    return tuple(
        (key, "present", inp[key]) if key in inp else (key, "absent")
...
def _success_leaves(inp, prepared):
    ...
    identity = (
        _raw_controls(inp), analysis.analysis_signature(inp), concrete_identity,
        material_ids, _runtime_laws(inp), _catalogs(inp),
    )
    leaves = []
...
def _invalid_leaves(inp, out):
    identity = (dict(out), _raw_controls(inp))
    leaves = []
    _flatten(identity, "invalid-fatigue-boundary", leaves)
    return tuple(leaves)
...
def _read(inp, out, context):
    ...
    return (
        success_plan(
            ...
            concrete_parameters_type=type(out["concrete_parameters"]).__name__,
            factors=(
                inp.get("fatigue_gamma_s"), inp.get("fatigue_gamma_ff"),
...
def _factor_result(value, label):
    if value is None:
        return TraceResult(RESULT_UNDEFINED, None, f"Entered {label} is absent")
    return float(value)


def _root_values(family):
    values = {}
    ...
    if family.factors is None:
        ...
        gamma_s, gamma_ff, gamma_c = family.factors
        values.update({
            "input-gamma-s": _factor_result(gamma_s, "gamma_s"),
            "input-gamma-ff": _factor_result(gamma_ff, "gamma_ff"),
            "input-gamma-c": _factor_result(gamma_c, "gamma_c"),
        })
...
def _assessment_values(family, plan, prepared, out):
    ...
    for bin_plan, state, row in zip(plan.bins, spectrum.bins, result.bins):
        ...
        position = plan.element_position
...
def _aggregate_values(family, out):
    values = _root_values(family)
    results = []
    for plan in family.assessments:
        result = next(
            row for row in out["spectra"][plan.spectrum_position].reinforcement
...
def _calculations(family, prepared, out):
    if family.branch == "invalid":
        ...
        return (_calculation(
            ...
            "Invalid fatigue boundary", family.invalid_axes, contracts, values,
            False,
    ...
    values, converged = _aggregate_values(family, out)
    calculations.append(_calculation(
        f"fatigue.{family.context_token}.reinforcement-output",
        "Reinforcement fatigue output", family.aggregate_axes,
        aggregate_steps(family), values, converged,
    ))
