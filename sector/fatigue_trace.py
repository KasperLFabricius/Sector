...
def _shape(actual: Any, expected: Any, label: str) -> None:
    """Pin recursive shape and concrete retained types, excluding values."""
...
def _section_vector(section: Any) -> Any:
    """Return geometry leaves without erasing their retained scalar types."""

    if section is None:
        return None
    return (
        tuple(tuple((point[0], point[1]) for point in ring)
...
class _Graph:
    ...
    def inputs(self, inp: Mapping[str, Any], success: bool) -> str:
        ...
        return self.add("normalised-fatigue-inputs",
                        "Closed normalised fatigue input identity", ONE,
    ...
    def factors(self, inp: Mapping[str, Any]):
        ...
        for key, title in (
            ...
            output.append(self.add(
                ...
                title, ONE, ROLE_USER_INPUT, INPUT))
        return tuple(output)
...
def _yield(stress: float, prop: ReinforcementFatigueProperties, gamma_s: float):
    characteristic = (prop.fytk_mpa if stress >= 0.0 else
                      prop.fyck_mpa if prop.fyck_mpa is not None else
                      prop.fytk_mpa)
...
def _calculation(member: MemberSpec, graph: _Graph, title: str, *,
                 failed: str | None) -> TraceCalculation:
    ...
    for node in member.nodes:
        result = _trace_result(
            ...
            failed if node.role == ROLE_FINAL else None)
        substituted = f"{node.step_id} = {result.state}"
        if result.value is not None:
            substituted = f"{node.step_id} = {result.value:.17g} {node.unit.symbol}"
        steps.append(TraceStep(
            node.step_id, node.title,
            tuple(TraceDependency(item, units[item]) for item in node.dependencies),
            node.role, node.source, node.step_id, node.unit,
            f"Bind or independently derive {node.title.lower()}", substituted,
            result))
    return TraceCalculation(
        member.calculation_id, COVERAGE_ID, title, METHOD_ID, member.axes,
        steps[-1].step_id, tuple(steps))


def _state_vectors(state: FatigueBinState, element_index: int):
    fatigue_total = (state.bar_stress_fatigue_total_mpa
                     or state.bar_stress_total_mpa)
    design_total = (state.bar_stress_fatigue_design_total_mpa
                    or fatigue_total)
    try:
        return (
...
def _bin_nodes(graph: _Graph, prefix: str, assessment: ReinforcementBinResult,
               ...
               gamma_ff_id: str, gamma_s: float, gamma_ff: float,
               sn_source, proof_source):
    ...
    for actual, wanted, label in (
        ...
        (assessment.stress_total_design_mpa, design_total,
         "design fatigue-total stress"),
        (assessment.stress_total_elastic_mpa, perfect_total,
         "perfect-bond stress"),
        (assessment.stress_range_mpa, stress_range, "stress range"),
        (assessment.stress_range_elastic_mpa, perfect_range,
         "perfect-bond range"),
        (assessment.bond_adjustment, bond_factor, "bond adjustment"),
        (assessment.design_stress_range_mpa, design_range,
         "design stress range"),
    ):
    ...
    bin_identity = graph.add(
        ...
        "Exact fatigue-bin name and description identity", ONE,
        ROLE_COMPUTED, ELASTIC, normal)
    cycles = graph.add(
        f"{prefix}-cycles", "Retained applied cycles", CYCLES,
        ROLE_COMPUTED, ELASTIC, normal, bin_identity, value=float(state.cycles))
    convergence = graph.add(
        f"{prefix}-converged", "Matched Elastic-bin convergence", ONE,
        ROLE_COMPUTED, ELASTIC, normal, bin_identity,
        value=float(state.converged))
    bond_identity = graph.add(
        f"{prefix}-bond-method-{_fingerprint(state.bond_method)}",
        "Exact retained bond-method identity", ONE, ROLE_COMPUTED, ELASTIC,
        normal, bin_identity)
    long_id = graph.add(
        f"{prefix}-long-stress", "Matched long-term reinforcement stress",
        MPA, ROLE_COMPUTED, ELASTIC, normal, bin_identity, value=long_stress)
    total_id = graph.add(
        f"{prefix}-fatigue-total-stress",
        "Matched bond-adjusted fatigue-total stress", MPA, ROLE_COMPUTED,
        ELASTIC, normal, bin_identity, value=total_stress)
    design_id = graph.add(
        f"{prefix}-design-fatigue-total-stress",
        "Matched design fatigue-total stress", MPA, ROLE_COMPUTED, ELASTIC,
        normal, bin_identity, gamma_ff_id, value=design_total)
    perfect_id = graph.add(
        f"{prefix}-perfect-bond-total-stress",
        "Matched perfect-bond total stress", MPA, ROLE_COMPUTED, ELASTIC,
        normal, bin_identity, value=perfect_total)
    range_id = graph.add(
        f"{prefix}-stress-range", "Bond-adjusted stress range", MPA,
        ROLE_COMPUTED, ELASTIC, total_id, long_id, bond_identity,
        value=stress_range)
    perfect_range_id = graph.add(
        f"{prefix}-perfect-bond-range", "Perfect-bond stress range", MPA,
        ROLE_COMPUTED, ELASTIC, perfect_id, long_id, value=perfect_range)
    bond_factor_id = graph.add(
        f"{prefix}-bond-adjustment", "Bond adjustment", ONE, ROLE_COMPUTED,
        ELASTIC, range_id, perfect_range_id, bond_identity, value=bond_factor)
    design_range_id = graph.add(
        f"{prefix}-design-stress-range", "Design stress range", MPA,
        ROLE_COMPUTED, ELASTIC, design_id, long_id, range_id, bond_factor_id,
        gamma_ff_id, value=design_range)

    reference_value = float(prop.delta_sigma_rsk_mpa)
    design_reference_value = reference_value / gamma_s
    _same_number(assessment.delta_sigma_rsk_mpa, reference_value,
    ...
    reference = graph.add(
        f"{prefix}-delta-sigma-rsk", "Characteristic reference range", MPA,
        ROLE_COMPUTED, sn_source, normal, value=reference_value)
    design_reference = graph.add(
        f"{prefix}-delta-sigma-rd", "Design reference range", MPA,
        ROLE_COMPUTED, sn_source, reference, gamma_s_id,
    ...
    exponent = graph.add(
        ...
        ROLE_COMPUTED, sn_source, design_range_id, design_reference,
        value=life.exponent)
    log_life = graph.add(
        f"{prefix}-log10-cycles-to-failure", "Logarithmic fatigue life", ONE,
        ROLE_COMPUTED, sn_source, design_range_id, design_reference, exponent,
        gamma_s_id, value=life.log10_cycles)
    life_id = graph.add(
        f"{prefix}-cycles-to-failure", "Cycles to failure", CYCLES,
        ROLE_COMPUTED, sn_source, log_life, value=life.cycles)
    ...
    damage = graph.add(
        ...
        ROLE_COMPUTED, sn_source, cycles, log_life, life_id, value=damage_value)
    ...
    if total_util >= long_util:
        governing_stress_value, limit_value, utilisation_value = (
            design_total, total_limit, total_util)
    else:
        governing_stress_value, limit_value, utilisation_value = (
            long_stress, long_limit, long_util)
    for actual, wanted, label in (
        (assessment.governing_stress_mpa, governing_stress_value,
         "governing stress"),
        (assessment.yield_limit_mpa, limit_value, "yield limit"),
        (assessment.yield_utilisation, utilisation_value,
         "yield utilisation"),
    ):
        _same_number(actual, wanted, f"{prefix} {label}")
    governing_stress = graph.add(
        f"{prefix}-governing-stress", "Governing absolute stress", MPA,
        ROLE_COMPUTED, proof_source, long_id, design_id,
        value=governing_stress_value)
    limit_id = graph.add(
        f"{prefix}-yield-limit", "Design proof-stress limit", MPA,
        ROLE_COMPUTED, proof_source, governing_stress, gamma_s_id,
        value=limit_value)
    utilisation = graph.add(
        f"{prefix}-yield-utilisation", "Yield utilisation", ONE,
        ROLE_COMPUTED, proof_source, governing_stress, limit_id,
        value=utilisation_value)
    return damage, utilisation, convergence
...
def _assessment_member(inp, spectrum: FatigueSpectrumResult,
    ...
    if (assessment.element_id != prop.element_id
    ...
    _same_number(assessment.diameter_mm, prop.diameter_mm,
    ...
    property_identity = graph.add(
        ...
        "Exact resolved reinforcement fatigue properties", ONE,
        ROLE_COMPUTED, BOUNDARY, normal)
    sn_source, proof_source = code_sources(
        _analysis().prepare(inp).edition, assessment.kind)
    ...
    for bin_index, (item, state) in enumerate(zip(assessment.bins,
                                                   spectrum.bins)):
        ...
        damage, yield_util, convergence = _bin_nodes(
            graph, prefix, item, state, prop, element_index, normal,
            gamma_s_id, gamma_ff_id, gamma_s, gamma_ff, sn_source,
            proof_source)
    ...
    damage_total = graph.add(
        "element-total-damage", "Element Palmgren-Miner damage", ONE,
        ROLE_COMPUTED, sn_source, *damages, value=total_damage)
    damage_util = graph.add(
        "element-damage-utilisation", "Element damage utilisation", ONE,
        ROLE_COMPUTED, sn_source, damage_total,
    ...
    damage_identity = graph.add(
        ...
        *damages)
    yield_max = graph.add(
        "element-yield-utilisation", "Element yield utilisation", ONE,
        ROLE_COMPUTED, proof_source, *yields, value=max_yield)
    ...
    convergence = graph.add(
        "element-converged", "All matched Elastic bins converged", ONE,
        ROLE_COMPUTED, ELASTIC, *convergences, value=float(converged))
    ...
    status = graph.add(
        "element-status", "Element PASS or FAIL", ONE, ROLE_COMPUTED,
        VERDICT, damage_total, yield_max, convergence, value=float(passed))
    graph.add(
...
def _aggregate_member(inp, spectra, context):
    ...
    for spectrum_index, spectrum in enumerate(spectra):
        ...
        governing = max(spectrum.reinforcement,
    ...
    governing = graph.add(
        ...
        "Governing reinforcement utilisation", ONE, ROLE_COMPUTED, VERDICT,
        *assessment_ids, value=utilisation)
    status = graph.add(
        "reinforcement-output-status", "Worst-first reinforcement status",
        ONE, ROLE_COMPUTED, VERDICT, *assessment_ids, value=float(passed))
    graph.add(
        "ct-010-reinforcement-output-result", "CT-010a reinforcement output",
        ONE, ROLE_FINAL, VERDICT, normal, gamma_s, gamma_ff, gamma_c,
        *assessment_ids, *governing_ids, governing, status, value=utilisation)
    member = MemberSpec(
        ...
        tuple(graph.nodes), ALL_FINAL_STATES)
    return member, _calculation(
        member, graph, "Reinforcement fatigue output",
        failed=None if converged else _FAILED)
...
def _invalid_member(inp, candidate, context):
    ...
    errors = [graph.add(
        ...
        "Retained invalid fatigue error", ONE, ROLE_COMPUTED, ELASTIC, payload)
        for index, error in enumerate(candidate["errors"])]
    graph.add(
        "ct-010-invalid-result", "CT-010 invalid fatigue state", ONE,
    ...
    return member, _calculation(
        member, graph, "Invalid fatigue input", failed=reason)


def _validate_boolean_controls(inp: Mapping[str, Any]) -> None:
    """Fail before dispatch so no branch can launder Boolean lookalikes."""

    for key in _BOOLEAN_CONTROLS:
        if type(inp.get(key)) is not bool:
            raise TraceValidationError(
                f"{key} must retain exact built-in Boolean type")


def _build_expected(inp, out, input_sha256, result_sha256, context):
    _validate_boolean_controls(inp)
    candidate = _mapping(out, "retained result mapping").get("fatigue")
    enabled = inp["fatigue_on"]
    if not enabled:
    ...
    if "valid" in candidate:
        ...
        if not inp["fatigue_check_steel"]:
            return None
