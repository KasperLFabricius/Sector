...
from .fatigue_trace_contract import (
    ALL_FINAL_STATES, BOUNDARY, COVERAGE_ID, CYCLES, ELASTIC, INPUT,
    INVALID_KEYS, METHOD_ID, MPA, ONE, RAW_CONTROLS, SUCCESS_KEYS, VERDICT,
    MemberSpec, NodeSpec, code_sources, registry_for,
)
...
def _exact(actual: Any, expected: Any, label: str) -> None:
    ...
    if dataclasses.is_dataclass(expected) and not isinstance(expected, type):
        for field in dataclasses.fields(expected):
            _exact(getattr(actual, field.name), getattr(expected, field.name),
                   f"{label}.{field.name}")
        return
...
def _shape(actual: Any, expected: Any, label: str) -> None:
    ...
    if dataclasses.is_dataclass(expected) and not isinstance(expected, type):
        for field in dataclasses.fields(expected):
            _shape(getattr(actual, field.name), getattr(expected, field.name),
                   f"{label}.{field.name}")
        return
    if isinstance(expected, np.ndarray):
        if actual.dtype != expected.dtype or actual.shape != expected.shape:
...
def _check_state(actual: Any, expected: FatigueBinState, label: str) -> None:
    ...
    for field in dataclasses.fields(expected):
        compare = _shape if field.name in excluded else _exact
        compare(getattr(actual, field.name), getattr(expected, field.name),
                f"{label}.{field.name}")


def _check_spectrum(actual: Any, expected: FatigueSpectrumResult, *,
                    concrete_enabled: bool, label: str) -> None:
    ...
    _exact(actual.governing_reinforcement_id,
           expected.governing_reinforcement_id,
           f"{label}.governing_reinforcement_id")
    for name in ("concrete", "concrete_search", "fcd_fat_mpa",
...
def _check_success(candidate: Any, expected: Mapping[str, Any]) -> dict[str, Any]:
    ...
    overall = {"governing_spectrum", "utilisation", "converged", "passed"}
    for key in SUCCESS_KEYS:
        if key == "spectra":
            ...
            for index, (item, wanted) in enumerate(zip(candidate[key], expected[key])):
                _check_spectrum(item, wanted, concrete_enabled=concrete_enabled,
                                label=f"candidate.spectra[{index}]")
        elif key == "calculation_references":
            actual_refs, wanted_refs = candidate[key], expected[key]
            if type(actual_refs) is not type(wanted_refs):
                raise TraceValidationError("calculation references retained type differs")
            if tuple(actual_refs) != tuple(wanted_refs):
                raise TraceValidationError("calculation reference keys/order differ")
            for name in wanted_refs:
                compare = _shape if name == "concrete" else _exact
                compare(actual_refs[name], wanted_refs[name],
                        f"candidate.calculation_references.{name}")
        elif key in concrete_values or (concrete_enabled and key in overall):
...
def _identity(value: Any) -> Any:
    ...
    if dataclasses.is_dataclass(value) and not isinstance(value, type):
        return ["dataclass", kind, [[field.name, _identity(getattr(value, field.name))]
                                     for field in dataclasses.fields(value)]]
    if isinstance(value, np.ndarray):
        return ["ndarray", kind, str(value.dtype), list(value.shape),
                _identity(value.tolist())]
    if kind == "pandas.core.frame.DataFrame":
        return ["dataframe", kind, _identity(tuple(value.columns)),
    ...
    if isinstance(value, Mapping):
        return ["mapping", kind, [[_identity(key), _identity(item)]
    ...
    if hasattr(value, "__dict__"):
        return ["object", kind, [[name, _identity(item)]
                                  for name, item in vars(value).items()]]
    return ["repr", kind, repr(value)]


def _fingerprint(value: Any) -> str:
    data = json.dumps(_identity(value), ensure_ascii=True,
...
def _section_vector(section: Any) -> Any:
    if section is None:
        return None
    return (
        tuple(tuple((float(x), float(y)) for x, y in ring)
...
def _identity_groups(inp: Mapping[str, Any], success: bool):
    ...
    groups.extend((
        ...
        ("prestress-material-catalog", inp.get("prestress_material_catalog", absent)),
...
class _Graph:
    ...
    def add(self, step_id, title, unit, role, source, *dependencies, value=1.0):
        self.nodes.append(NodeSpec(step_id, title, unit, role, source,
                                   tuple(dependencies)))
    ...
    def inputs(self, inp: Mapping[str, Any], success: bool) -> str:
        leaves = [self.add(
            ...
            f"Exact retained {label.replace('-', ' ')} identity", ONE,
            ROLE_USER_INPUT, INPUT,
        ) for label, identity in _identity_groups(inp, success)]
        return self.add("normalised-fatigue-inputs",
                        "Complete retained fatigue input identity", ONE,
                        ROLE_COMPUTED, BOUNDARY, *leaves)
    ...
    def factors(self, inp: Mapping[str, Any]):
        ...
        for step_id, key, title in (
            ...
            raw = inp.get(key)
            value = float(raw) if type(raw) in {int, float} and type(raw) is not bool else None
            output.append(self.add(step_id, title, ONE, ROLE_USER_INPUT, INPUT,
                                   value=value))
        return tuple(output)


def _same_number(actual: Any, expected: float, label: str) -> None:
    if type(actual) is not float or not _float_same(actual, float(expected)):
...
def _yield(stress: float, prop: ReinforcementFatigueProperties, gamma_s: float):
    characteristic = (prop.fytk_mpa if stress >= 0.0 or prop.fyck_mpa is None
                      else prop.fyck_mpa)
...
def _trace_result(value: Any, failed: str | None = None) -> TraceResult:
    ...
    if math.isinf(number):
        return TraceResult(RESULT_POSITIVE_INFINITY if number > 0.0
                           else RESULT_NEGATIVE_INFINITY, None, _UNBOUNDED)
    return TraceResult(RESULT_FINITE, number)


def _calculation(member: MemberSpec, graph: _Graph, title: str, *,
                 failed: str | None = None) -> TraceCalculation:
    units = {node.step_id: node.unit for node in member.nodes}
    final_id = member.nodes[-1].step_id
    steps = []
    for node in member.nodes:
        result = _trace_result(graph.values[node.step_id],
                               failed if node.step_id == final_id else None)
        shown = result.state if result.state != RESULT_FINITE else (
            f"{result.value:.17g} {node.unit.symbol}")
        steps.append(TraceStep(
            node.step_id, node.title,
            tuple(TraceDependency(name, units[name]) for name in node.dependencies),
            node.role, node.source, node.step_id, node.unit,
            "Retained value with exact declared dependency closure",
            f"{node.step_id} = {shown}", result,
        ))
    return TraceCalculation(member.calculation_id, COVERAGE_ID, title, METHOD_ID,
                            member.axes, final_id, tuple(steps))


def _state_vectors(state: FatigueBinState, element_index: int):
    total = state.bar_stress_fatigue_total_mpa or state.bar_stress_total_mpa
    design = state.bar_stress_fatigue_design_total_mpa or total
    vectors = (state.bar_stress_long_mpa, total, design,
               state.bar_stress_total_mpa)
    if any(element_index >= len(vector) for vector in vectors):
        raise TraceValidationError(
            "matched Elastic bin lacks the reinforcement element index")
    return tuple(float(vector[element_index]) for vector in vectors)


def _bin_nodes(graph: _Graph, prefix: str, assessment: ReinforcementBinResult,
               state: FatigueBinState, prop: ReinforcementFatigueProperties,
               element_index: int, normal: str, gamma_s_id: str,
               gamma_ff_id: str, gamma_s: float, sn_source, yield_source):
    long_stress, fatigue_total, design_total, elastic_total = _state_vectors(
        state, element_index)
    for actual, wanted, label in (
        (assessment.cycles, state.cycles, "cycles"),
        (assessment.stress_long_mpa, long_stress, "long stress"),
        (assessment.stress_total_mpa, fatigue_total, "fatigue total stress"),
        (assessment.stress_total_design_mpa, design_total, "design stress"),
        (assessment.stress_total_elastic_mpa, elastic_total, "perfect-bond stress"),
    ):
        _same_number(actual, wanted, f"{prefix} {label}")
    if type(assessment.converged) is not bool or assessment.converged != state.converged:
        raise TraceValidationError(f"{prefix} convergence differs from Elastic")
    if assessment.bond_method != state.bond_method:
        raise TraceValidationError(f"{prefix} bond method differs from Elastic")

    cycles = graph.add(f"{prefix}-cycles", "Applied cycles", CYCLES,
                       ROLE_COMPUTED, ELASTIC, normal, value=state.cycles)
    long_id = graph.add(f"{prefix}-elastic-long-stress",
                        "Matched Elastic long stress", MPA, ROLE_COMPUTED,
                        ELASTIC, normal, value=long_stress)
    total_id = graph.add(f"{prefix}-elastic-fatigue-total-stress",
                         "Matched Elastic fatigue total stress", MPA,
                         ROLE_COMPUTED, ELASTIC, normal, value=fatigue_total)
    design_id = graph.add(f"{prefix}-elastic-design-total-stress",
                          "Matched Elastic action-factored total stress", MPA,
                          ROLE_COMPUTED, ELASTIC, normal, gamma_ff_id,
                          value=design_total)
    perfect_id = graph.add(f"{prefix}-elastic-perfect-bond-total-stress",
                           "Matched Elastic perfect-bond total stress", MPA,
                           ROLE_COMPUTED, ELASTIC, normal, value=elastic_total)
    convergence = graph.add(f"{prefix}-elastic-converged",
                            "Matched Elastic convergence", ONE, ROLE_COMPUTED,
                            ELASTIC, normal, value=float(state.converged))
    bond_identity = graph.add(f"{prefix}-bond-{_fingerprint(state.bond_method)}",
                              "Exact matched Elastic bond-method identity", ONE,
    ...
    bond_factor = (stress_range / elastic_range if elastic_range > 0.0
                   else math.inf if stress_range > 0.0 else 1.0)
    for actual, wanted, label in (
        (assessment.stress_range_mpa, stress_range, "stress range"),
        (assessment.stress_range_elastic_mpa, elastic_range, "perfect-bond range"),
        (assessment.design_stress_range_mpa, design_range, "design range"),
        (assessment.bond_adjustment, bond_factor, "bond adjustment"),
    ):
        _same_number(actual, wanted, f"{prefix} {label}")
    range_id = graph.add(f"{prefix}-stress-range", "Bond-adjusted stress range",
                         MPA, ROLE_COMPUTED, ELASTIC, total_id, long_id,
                         bond_identity, value=stress_range)
    elastic_range_id = graph.add(f"{prefix}-perfect-bond-range",
                                 "Perfect-bond stress range", MPA, ROLE_COMPUTED,
                                 ELASTIC, perfect_id, long_id, value=elastic_range)
    bond_factor_id = graph.add(f"{prefix}-bond-adjustment", "Bond adjustment",
                               ONE, ROLE_COMPUTED, ELASTIC, range_id,
                               elastic_range_id, bond_identity, value=bond_factor)
    design_range_id = graph.add(f"{prefix}-design-stress-range",
                                "Design stress range", MPA, ROLE_COMPUTED,
                                ELASTIC, design_id, long_id, range_id,
                                bond_factor_id, gamma_ff_id, value=design_range)
    reference = graph.add(f"{prefix}-delta-sigma-rsk",
                          "Characteristic reference range", MPA, ROLE_COMPUTED,
                          sn_source, normal, value=prop.delta_sigma_rsk_mpa)
    design_reference_value = prop.delta_sigma_rsk_mpa / gamma_s
    _same_number(assessment.delta_sigma_rsk_mpa, prop.delta_sigma_rsk_mpa,
                 f"{prefix} characteristic reference")
    _same_number(assessment.delta_sigma_rd_mpa, design_reference_value,
                 f"{prefix} design reference")
    design_reference = graph.add(f"{prefix}-delta-sigma-rd",
                                 "Design reference range", MPA, ROLE_COMPUTED,
                                 sn_source, reference, gamma_s_id,
                                 value=design_reference_value)
    life = steel_fatigue_life(
        design_range, n_star=prop.n_star, k1=prop.k1, k2=prop.k2,
        delta_sigma_rsk_mpa=prop.delta_sigma_rsk_mpa, gamma_s=gamma_s,
        gamma_ff=1.0)
    for actual, wanted, label in (
        ...
        _same_number(actual, wanted, f"{prefix} {label}")
    exponent = graph.add(f"{prefix}-sn-exponent", "Selected S-N exponent", ONE,
                         ROLE_COMPUTED, sn_source, design_range_id,
                         design_reference, value=life.exponent)
    log_life = graph.add(f"{prefix}-log10-cycles-to-failure",
                         "Logarithmic fatigue life", ONE, ROLE_COMPUTED,
                         sn_source, design_range_id, design_reference, exponent,
                         gamma_s_id, value=life.log10_cycles)
    life_id = graph.add(f"{prefix}-cycles-to-failure", "Cycles to failure",
                        CYCLES, ROLE_COMPUTED, sn_source, log_life,
                        value=life.cycles)
    damage_value = _damage(state.cycles, life.log10_cycles)
    _same_number(assessment.damage, damage_value, f"{prefix} damage")
    damage = graph.add(f"{prefix}-damage", "Palmgren-Miner bin damage", ONE,
                       ROLE_COMPUTED, sn_source, cycles, log_life, life_id,
                       value=damage_value)
    ...
    if total_util >= long_util:
        gov_stress, limit, util = design_total, total_limit, total_util
    else:
        gov_stress, limit, util = long_stress, long_limit, long_util
    for actual, wanted, label in (
        (assessment.governing_stress_mpa, gov_stress, "governing stress"),
        (assessment.yield_limit_mpa, limit, "yield limit"),
        (assessment.yield_utilisation, util, "yield utilisation"),
    ):
        _same_number(actual, wanted, f"{prefix} {label}")
    governing_stress = graph.add(f"{prefix}-governing-stress",
                                 "Governing absolute stress", MPA,
                                 ROLE_COMPUTED, yield_source, long_id, design_id,
                                 value=gov_stress)
    limit_id = graph.add(f"{prefix}-yield-limit", "Design proof-stress limit",
                         MPA, ROLE_COMPUTED, yield_source, governing_stress,
                         gamma_s_id, value=limit)
    yield_utilisation = graph.add(f"{prefix}-yield-utilisation",
                                  "Yield utilisation", ONE, ROLE_COMPUTED,
                                  yield_source, governing_stress, limit_id,
                                  value=util)
    return damage, yield_utilisation, convergence


def _assessment_member(inp, spectrum: FatigueSpectrumResult,
                       assessment: ReinforcementFatigueResult,
                       prop: ReinforcementFatigueProperties,
                       spectrum_index: int, element_index: int, context):
    graph = _Graph()
    normal = graph.inputs(inp, True)
    gamma_s_id, gamma_ff_id, gamma_c_id = graph.factors(inp)
    property_identity = graph.add(f"reinforcement-property-{_fingerprint(prop)}",
                                  "Exact resolved reinforcement fatigue properties",
                                  ONE, ROLE_COMPUTED, BOUNDARY, normal)
    ...
    for bin_index, (item, state) in enumerate(zip(assessment.bins, spectrum.bins)):
        ...
        damage, yield_util, convergence = _bin_nodes(
            graph, prefix, item, state, prop, element_index, normal, gamma_s_id,
            gamma_ff_id, float(inp["fatigue_gamma_s"]), sn_source, yield_source)
    ...
    _same_number(assessment.damage_utilisation, total_damage,
                 "element damage utilisation")
    _same_number(assessment.yield_utilisation, max_yield,
                 "element yield utilisation")
    damage_row = max(assessment.bins, key=lambda item: item.damage)
    yield_row = max(assessment.bins, key=lambda item: item.yield_utilisation)
    ...
    damage_total = graph.add("element-total-damage", "Element Palmgren-Miner damage",
                             ONE, ROLE_COMPUTED, sn_source, *damages,
                             value=total_damage)
    damage_util = graph.add("element-damage-utilisation",
                            "Element damage utilisation", ONE, ROLE_COMPUTED,
                            sn_source, damage_total,
    ...
    damage_identity = graph.add(
        ...
        "Exact governing damage-bin identity", ONE, ROLE_COMPUTED, VERDICT,
        *damages)
    yield_max = graph.add("element-yield-utilisation",
                          "Element yield utilisation", ONE, ROLE_COMPUTED,
                          yield_source, *yields, value=max_yield)
    yield_identity = graph.add(
        f"governing-yield-bin-{trace_identity_token(yield_row.bin_name)}",
        "Exact governing yield-bin identity", ONE, ROLE_COMPUTED, VERDICT,
        *yields)
    ...
    convergence = graph.add("element-converged",
                            "All matched Elastic bins converged", ONE,
                            ROLE_COMPUTED, ELASTIC, *convergences,
    ...
    status = graph.add("element-status", "Element PASS or FAIL", ONE,
                       ROLE_COMPUTED, VERDICT, damage_total, yield_max,
                       convergence, value=float(passed))
    graph.add("ct-010-reinforcement-assessment-result",
              "CT-010a reinforcement assessment utilisation", ONE, ROLE_FINAL,
              VERDICT, normal, gamma_s_id, gamma_ff_id, gamma_c_id,
              property_identity, damage_total, damage_util, damage_identity,
              yield_max, yield_identity, convergence, status, value=utilisation)
    member = MemberSpec(
        ...
        f"{trace_identity_token(spectrum.spectrum_name)}-"
        f"{trace_identity_token(assessment.element_id)}",
        context_axes(context, fatigue_spectrum=spectrum.spectrum_name,
                     reinforcement_element=assessment.element_id,
                     reinforcement_kind=assessment.kind,
                     fatigue_detail=assessment.detail_id),
        tuple(graph.nodes), ALL_FINAL_STATES)
    return member, _calculation(
        member, graph,
        f"Reinforcement fatigue: {spectrum.spectrum_name} / {assessment.element_id}",
...
def _aggregate_member(inp, spectra, context):
    ...
    assessment_ids, governing_ids, results = [], [], []
    for spectrum_index, spectrum in enumerate(spectra):
        ...
        for element_index, item in enumerate(spectrum.reinforcement):
            step_id = graph.add(
                ...
                "Retained reinforcement assessment utilisation", ONE,
                ROLE_COMPUTED, VERDICT, normal, value=item.utilisation)
        ...
        governing = max(spectrum.reinforcement, key=lambda item: item.utilisation)
        ...
        governing_ids.append(graph.add(
            ...
            "Exact governing reinforcement identity", ONE, ROLE_COMPUTED,
            VERDICT, *spectrum_ids))
    if not results:
    ...
    governing = graph.add("governing-reinforcement-utilisation",
                           "Governing reinforcement utilisation", ONE,
                           ROLE_COMPUTED, VERDICT, *assessment_ids,
                           value=utilisation)
    status = graph.add("reinforcement-output-status",
                       "Worst-first reinforcement status", ONE, ROLE_COMPUTED,
                       VERDICT, *assessment_ids, value=float(passed))
    graph.add("ct-010-reinforcement-output-result",
              "CT-010a reinforcement output", ONE, ROLE_FINAL, VERDICT,
              normal, gamma_s, gamma_ff, gamma_c, *assessment_ids,
              *governing_ids, governing, status, value=utilisation)
    member = MemberSpec(
        "reinforcement-output",
        f"ct-010-{context_id(context)}-reinforcement-output",
        context_axes(context, fatigue_branch="reinforcement-output",
                     assessment_count=str(len(results))),
        tuple(graph.nodes), ALL_FINAL_STATES)
    return member, _calculation(member, graph, "Reinforcement fatigue output",
                                failed=None if converged else _FAILED)
...
def _invalid_member(inp, candidate, context):
    ...
    payload = graph.add(f"invalid-output-{_fingerprint(candidate)}",
                        "Exact retained invalid fatigue output identity", ONE,
                        ROLE_COMPUTED, ELASTIC, normal)
    errors = [graph.add(f"error-{index:02d}-{trace_identity_token(error)}",
                        "Retained invalid fatigue error", ONE, ROLE_COMPUTED,
                        ELASTIC, payload)
              for index, error in enumerate(candidate["errors"])]
    graph.add("ct-010-invalid-result", "CT-010 invalid fatigue state", ONE,
              ROLE_FINAL, VERDICT, normal, payload, *errors, value=0.0)
    member = MemberSpec(
        "invalid", f"ct-010-{context_id(context)}-invalid",
        context_axes(context, fatigue_branch="invalid",
                     error_count=str(len(errors))),
        tuple(graph.nodes), frozenset({RESULT_FAILED}))
    reason = "Retained fatigue invalid result."
    if candidate["errors"]:
        reason += " " + " | ".join(candidate["errors"])
    return member, _calculation(member, graph, "Invalid fatigue input",
                                failed=reason)
...
def _build_expected(inp, out, input_sha256, result_sha256, context):
    ...
    if "valid" in candidate:
        ...
        members, calculations = (member,), (calculation,)
    else:
        ...
        for spectrum_index, spectrum in enumerate(spectra):
            for element_index, assessment in enumerate(spectrum.reinforcement):
                ...
                member, calculation = _assessment_member(
                    inp, spectrum, assessment, prop, spectrum_index,
                    element_index, context)
                member_list.append(member)
                calculation_list.append(calculation)
        member, calculation = _aggregate_member(inp, spectra, context)
        member_list.append(member)
        calculation_list.append(calculation)
        members, calculations = tuple(member_list), tuple(calculation_list)
    bundle = create_bundle(input_sha256=input_sha256,
...
def build_fatigue_trace_family(
    ...
    try:
        return _build_expected(
            ...
            _mapping(out, "retained result mapping"), input_sha256,
            result_sha256,
        ...
        raise TraceValidationError(
            f"invalid CT-010a fatigue evidence: {exc}") from exc


def validate_fatigue_trace_family(
    bundle: TraceBundle | dict[str, Any] | None, inp: Mapping[str, Any],
