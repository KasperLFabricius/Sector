"""CT-009 contract for the EN 1992-1-1:2004 base crack branch."""
...
def standard(method: str, clause: str, locator: str) -> TraceSource:
    return TraceSource(
...
EFFECTIVE_AREA = standard(
    "en-1992-1-1-2004-effective-tension-area", "7.3.2(3)",
    "Figure 7.1 and h_c,eff definition",
)
MEAN_STRAIN = standard(
    "en-1992-1-1-2004-mean-strain-difference", "7.3.4(2)",
    "Expression (7.9)",
)
SPACING_CLOSE = standard(
    "en-1992-1-1-2004-close-centre-crack-spacing", "7.3.4(3)",
    "Expression (7.11)",
)
SPACING_WIDE = standard(
    "en-1992-1-1-2004-wide-spacing-crack-spacing", "7.3.4(4)",
    "Expression (7.14)",
)
CRACK_WIDTH = standard(
    "en-1992-1-1-2004-crack-width", "7.3.4(1)", "Expression (7.8)",
)
    ...
    for member in members:
        rows.append(TraceMemberContract(
            ...
            sources=frozenset(
                ...
                for _step_id, _role, source, _dependencies in member.steps
            ),
            result_states=member.result_states,
            step_ids=tuple(row[0] for row in member.steps),
            step_dependencies=tuple((row[0], row[3]) for row in member.steps),
            step_metadata=tuple(
                TraceStepMetadataContract(row[0], row[1], row[2])
                for row in member.steps
            ),
        ))
    return TraceRegistryContract(
        REGISTRY_ID, (TraceFamilyContract(FAMILY_ID, tuple(rows)),),
    )


SUCCESS_STATES = frozenset({RESULT_FINITE, RESULT_POSITIVE_INFINITY})
