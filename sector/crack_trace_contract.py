...
def _case_inputs(rows, shape: CrackShape, long_case: bool):
    ...
    upstream = [
        rows.add(f"upstream-plane-{name}", f"CT-005 cracked-plane {name}",
                 unit, ROLE_METHOD_VALUE, UPSTREAM_SOURCE)
    ...
    for index, element in enumerate(shape.elements):
        upstream.append(rows.add(
            ...
            "CT-005 cracked-state element stress", MPA, ROLE_METHOD_VALUE,
            UPSTREAM_SOURCE))
...
def _variant_steps(rows, shape: CrackShape, variant: VariantShape,
    ...
    for candidate in variant.candidates:
        ...
        sigma = rows.add(f"{c}-sigma-s", "Stage II steel stress", MPA,
                         ROLE_COMPUTED, STRAIN_SOURCE,
                         f"upstream-{prefix}-stress")
...
def expected_case_steps(shape: CrackShape, case: CaseShape) -> tuple[StepSpec, ...]:
    rows = _Rows()
    normalised = _case_inputs(rows, shape, case.name == "long-term")
    states = [_variant_steps(rows, shape, variant, normalised)
