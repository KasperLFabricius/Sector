...
def _evaluate_combined_prerequisites(
    ...
    *,
    ...
    interaction_authority = combined_interaction_authority(inp, torsion_out)
    links_required = interaction_authority.links_required
    ...
    if not isinstance(torsion_out, Mapping):
        ...
        elif links_required and not interaction_authority.retained_current:
            torsion_valid = False
            reasons.append(
    ...
    if require_final_evidence:
        ...
        if links_required and not combined_longitudinal_chord_evidence_is_valid(
            links,
            shear_axis=inp.get("shear_axis"),
            shear_tension_low=inp.get("shear_tension"),
            ...
            links_valid = False
            reasons.append(
                "active longitudinal chord evidence is missing, malformed, "
                "or incomplete"
            )
        ...
        if torsion_subdivided is False and torsion_live:
    ...
    if require_final_evidence and interaction_authority.interaction_required:
        links = shear_out.get("links") if isinstance(shear_out, Mapping) else None
        link_res = links.get("res") if isinstance(links, Mapping) else None
    ...
    if require_final_evidence and interaction_authority.interaction_required:
        interaction = (
            torsion_out.get("interaction", _MISSING)
            if isinstance(torsion_out, Mapping)
            else _MISSING
        )
        if interaction is _MISSING or interaction is None:
            interaction_malformed = True
        else:
            links = shear_out.get("links") if isinstance(shear_out, Mapping) else None
            link_res = links.get("res") if isinstance(links, Mapping) else None
