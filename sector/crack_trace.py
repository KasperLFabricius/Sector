...
Upstream binding: the short-term crack case CONSUMES the published,
CT-005-validated surfaces on ``out["elastic"]`` (stress_plane, total) as its
bound upstream leaves; only values genuinely read from the published result
carry the upstream-evidence source. The long-term cracked state at nl with
beta 0.5 is not published, so it is re-derived through the retained kernels
(``analyse_cracking``/``combined_cracking``/``evaluate_crack_width`` with the
retained app argument mapping, in the retained composition order of
``app/sector_app.py``) and truthfully labeled as project replay - and that
rerun is CROSS-CHECKED against every published validated surface (total/
long/dif/rst1 stresses, stress_plane, converged, cracked, lambda_cr,
sigma_ct, fctm): any divergence between the retained evidence and the
currently imported solver fails closed before anything is sealed. Crack
widths are output-only quantities (Product Identity): finals aggregate the
retained calculation states (CALCULATED / NOT ASSESSED / NOT APPLICABLE /
INVALID) and never a PASS/FAIL verdict.
"""
...
def _replay(inp, context):
    ...
    replay = dict(
        ...
        tendon_is=[dict(block.values)["IS"] for block in blocks.tendons],
...
def _case_evidence(replay, name, context, shape_elements):
    ...
    # Short case: the upstream leaves are genuinely READ from the published,
    # CT-005-validated surfaces (stress_plane, total). Long case: the cracked
    # state at nl/beta 0.5 is not published, so its leaves carry the
    # cross-checked replay values under the project-replay source.
    if name == "short-term":
        published = replay["published"]
        plane_values = [float(value) for value in published["stress_plane"]]
        stresses = [float(value) for value in published["total"]]
    else:
        plane = replay["planes"][name]
        plane_values = [float(plane.eps0), float(plane.kx), float(plane.ky)]
        stresses = [float(value) for value in
                    np.asarray(plane.bar_stress, dtype=float) / 1000.0]
    values: dict[str, Any] = {
        ...
        "upstream-plane-eps0": plane_values[0],
        "upstream-plane-kx": plane_values[1],
        "upstream-plane-ky": plane_values[2],
    }
    if state["dk"]:
    ...
    for index, element in enumerate(shape_elements):
        ...
        if element.kind == "tendon":
            values[f"{prefix}-is"] = float(replay["tendon_is"][index - nb])
        values[f"upstream-{prefix}-stress"] = stresses[index]
    codes = []
...
def _output_evidence(replay, context):
    ...
    values: dict[str, Any] = {
        ...
        # Genuinely read from the published, cross-checked CT-005 surface.
        "upstream-converged": float(replay["published"]["converged"]),
    }
...
def _validate_surfaces(eout, replay):
    ...
    # The cross-checked rerun must reproduce EVERY published CT-005-validated
    # stress surface exactly; any solver divergence fails closed here before
    # a single upstream or replay leaf is sealed.
    for key, attribute in (("total", "bar_stress_total"),
                           ("long", "bar_stress_long"),
                           ("dif", "bar_stress_dif"),
                           ("rst1", "bar_stress_rst1")):
        published_row = _sequence(eout.get(key), f"published {key} stresses")
        wanted_row = np.asarray(
            getattr(replay["result"], attribute), dtype=float) / 1000.0
        if len(published_row) != wanted_row.size:
            raise TraceValidationError(
                f"published {key} stress cardinality differs")
        for index, wanted in enumerate(wanted_row):
            _compare_retained(published_row[index], float(wanted),
                              f"published {key} stress {index}")
    ...
    _compare_retained(eout.get("sigma_ct"), float(replay["sigma_ct"]),
                      "published first-cracking stress")
...
def _expected_bundle(inp, out, input_sha256, result_sha256, context):
    ...
    if not replay["active"]:
        return None
    # Only cross-checked published surfaces are consumed as upstream leaves.
    replay["published"] = eout
