...
_ROUTE_IDENTITIES = {
    ...
    (False, "building-2023"): _RouteIdentity(
        CODE_2023,
        EDITION_2023,
        METHOD_2023_APPLICABILITY,
        "building-2023",
...
def _active_route(inp: Mapping[str, Any]) -> tuple[bool, str | None] | None:
    ...
    routes = {
        ...
        (CODE_2023, EDITION_2023, False): (False, "building-2023"),
    }
...
def _input_identity(
    ...
    tendon_xi_active = route == "building-2023" and bool(blocks.tendons)
    inert = frozenset(
...
def _reconstruct(
    ...
    if route == "building-2023":
        for material in (blocks.concrete, *blocks.bars, *blocks.tendons):
            provenance = material.provenance.source
            documents = (
                provenance.edition,
                None if provenance.citation is None else provenance.citation.document,
            )
            if any(document is not None and "2023" in document for document in documents):
                raise TraceValidationError(
    ...
    member = _required(inp, "sls_member")
    bond_ratio_xi = None
    if route == "building-2023" and blocks.tendons:
        tendon_xi = _number(_required(inp, "sls_tendon_xi"), "sls_tendon_xi")
        if tendon_xi <= 0.0 or tendon_xi > 1.0:
            raise TraceValidationError(
                "sls_tendon_xi must be greater than zero and at most one for 2023 tendons"
            )
        bond_ratio_xi = [1.0] * len(blocks.bars) + [tendon_xi] * len(blocks.tendons)
...
def _candidate_steps_2023(
    ...
    *,
    ...
    """Expose every used 2023 kernel intermediate on a final-reaching path."""
    ...
    hc_step = created(
        ...
        "EN 1992-1-1:2023 Figure 9.3 effective height",
    )
    ac_step = created(
        "ac-eff", "Effective tension area", candidate.ac_eff, SQUARE_METRE,
        EFFECTIVE_AREA_2023, (*dependencies, hc_step),
        "EN 1992-1-1:2023 Figure 9.3 effective area",
    )
    bc_steps: list[TraceStep] = []
    if candidate.direct_tension:
        bc_steps.append(created(
            "bc-eff", "Direct-tension effective width", candidate.bc_ef,
            METRE, EFFECTIVE_AREA_2023, (*dependencies, hc_step, ac_step),
            "Solid-rectangle perimeter strip width from Figure 9.3",
        ))
    as_step = created(
        ...
        "Area of mild reinforcement in the effective tension area",
    )
    ap_step = created(
        "ap-eff", "Effective prestressing reinforcement area", candidate.ap_eff,
        SQUARE_METRE, EFFECTIVE_RATIO_2023, (*dependencies, ac_step, area_step),
        "Area of prestressing reinforcement in the effective tension area",
    )
    xi_steps: list[TraceStep] = []
    if candidate.xi1 is not None:
        xi_steps.append(created(
            "xi1", "Prestress bond conversion factor", candidate.xi1, ONE,
            BOND_RATIO_2023, (*dependencies, ap_step),
            "xi1 from the retained tendon bond-strength ratio",
        ))
    weighted_step = created(
        "ap-eff-weighted", "Bond-weighted prestressing area",
        candidate.ap_eff_weighted, SQUARE_METRE, EFFECTIVE_RATIO_2023,
        (*dependencies, ap_step, *xi_steps), "Sum xi1 times Ap in Ac,eff",
    )
    rho_step = created(
        ...
        "rho_p,eff = (As + sum(xi1 Ap)) / Ac,eff",
    )
    sigma_step = created(
    ...
    cover_step = created(
        ...
        "Distance to the concrete boundary minus reinforcement radius",
    )
    kw_step = created(
        "kw", "Calculated-width factor", candidate.kw, ONE, CRACK_WIDTH_2023,
        dependencies, "EN 1992-1-1:2023 Formula (9.8) NOTE 1 kw = 1.7",
    )
    k1r_step = created(
        "k1-r", "Curvature factor", candidate.k1_r, ONE, CURVATURE_2023,
        (*dependencies, hc_step, x_step, y_step),
        "EN 1992-1-1:2023 Formula (9.9)",
    )
    flexural_source = (
        DIRECT_FLEXURAL_2023 if candidate.direct_tension else FLEXURAL_2023
    )
    kfl_step = created(
        "kfl", "Flexural coefficient", candidate.kfl, ONE, flexural_source,
        (*dependencies, hc_step),
        (
            "EN 1992-1-1:2023 Formula (9.20) pure tension"
            if candidate.direct_tension
            else "EN 1992-1-1:2023 Formula (9.17) bending"
        ),
    )
    kb = 0.9 if selected_k1 <= 1.0 else 1.2
    kb_step = created(
        "kb", "Bond factor", kb, ONE, BOND_2023, dependencies,
        "EN 1992-1-1:2023 Formula (9.18) from selected k1 bond class",
    )
    strain_step = created(
        "esm-ecm", "Mean strain difference", candidate.esm_ecm, ONE,
        MEAN_STRAIN_2023, (*dependencies, rho_step, sigma_step),
        "EN 1992-1-1:2023 Formula (9.11)",
    )
    spacing_step = created(
        "sr-max", "Calculated mean crack spacing", candidate.sr_max,
        MILLIMETRE, SPACING_2023,
        (*dependencies, cover_step, phi_step, rho_step, kfl_step, kb_step),
        "EN 1992-1-1:2023 Formula (9.15)",
    )
    steps = [
        position_step, x_step, y_step, area_step, hc_step, ac_step, *bc_steps, as_step,
        ap_step, *xi_steps, weighted_step, rho_step, sigma_step, phi_step,
        cover_step, kw_step, k1r_step, kfl_step, kb_step, strain_step,
        spacing_step,
    ]
    categorical = (
        candidate.reinforcement_type,
    ...
    identity_steps = _identity_steps(
        ((f"{prefix}-method-identity", categorical),),
        source=ROUTE_2023,
        role=ROLE_COMPUTED,
        dependencies=(*dependencies, *steps),
        prefix=prefix,
    ...
    width = created(
        "wk", "Candidate calculated crack width", candidate.wk,
        MILLIMETRE, CRACK_WIDTH_2023,
        (kw_step, k1r_step, spacing_step, strain_step, *identity_steps),
...
def _case_member(
    ...
    is_2023 = replay.route == "building-2023"
    if case.evaluation.result is None:
        ...
        method_id = METHOD_2023_APPLICABILITY if is_2023 else route_identity.method_id
        direction = branch if is_2023 else "dominant-strain-gradient"
        scope = branch
        ...
        for position, candidate in enumerate(case.evaluation.result.candidates, start=1):
            if is_2023:
                selected_k1 = (
                    controls["sls_k1"]
                    if candidate.bar_index < len(replay.blocks.bars)
                    else 1.6
                )
                created, candidate_final = _candidate_steps_2023(
                    candidate,
                    position,
                    mechanics_roots,
                    selected_k1=float(selected_k1),
                )
            else:
        ...
        bond_range_steps: list[TraceStep] = []
        if is_2023 and case.evaluation.result.xi1_min is not None:
            bond_range_steps = [
                _step(
                    f"effective-xi1-{bound}",
                    f"Effective prestress bond factor {bound}",
                    float(value),
                    ONE,
                    ROLE_COMPUTED,
                    BOND_RATIO_2023,
                    candidate_finals,
                    expression=(
                        f"{bound.title()} xi1 over tendons in the effective area"
                    ),
                )
                for bound, value in (
                    ("minimum", case.evaluation.result.xi1_min),
                    ("maximum", case.evaluation.result.xi1_max),
                )
            ]
            steps.extend(bond_range_steps)
        final = _step(
            "crack-width-result",
            ...
            ROLE_FINAL,
            SELECTION,
            (*mechanics_roots, *candidate_finals, *bond_range_steps),
        ...
        direct = all(
            ...
            for candidate in case.evaluation.result.candidates
        )
        method_id = (
            METHOD_2023_DIRECT if direct else METHOD_2023_BENDING
        ) if is_2023 else route_identity.method_id
        direction = (
            CRACK_SCOPE_DIRECT_TENSION if direct else "dominant-strain-gradient"
        )
        scope = (
            CRACK_SCOPE_DIRECT_TENSION
            if direct
            else "refined-bending"
        )
        warnings = (
            (CRACK_DIRECT_TENSION_SCOPE,)
            if is_2023 and direct
...
def _aggregate_member(
    ...
    if is_2023:
        axis_values["crack_scope"] = "aggregate"
    if route_identity.axis is not None:
        axis_values["crack_route"] = route_identity.axis
    axes = context_axes(context, **axis_values)
    calculation_id = (
