...
def _assert_2023_number(actual: Any, expected: Any, label: str) -> None:
    if not math.isclose(
...
def _candidate_ratio_2023(
    ...
    *,
    ...
    areas = np.asarray([item.area for item in elements], dtype=float)
    direct_area = None
    if direct:
        direct_area = _direct_tension_area_2023(section, diameters)
        if isinstance(direct_area, str):
            raise TraceValidationError(
                "2023 direct-tension geometry contradicts a calculated candidate"
            )
        in_area = direct_area.in_area
        ac_eff = direct_area.ac_eff
        _assert_2023_number(candidate.ac_eff, ac_eff, "direct effective area")
        _assert_2023_number(
            candidate.hc_ef, direct_area.hc_eff, "direct effective height"
        )
        _assert_2023_number(
            candidate.bc_ef, direct_area.bc_eff, "direct effective width"
        )
    else:
    ...
    for actual, expected, label in (
        (candidate.as_eff, ratio.as_eff, "effective mild area"),
        (candidate.ap_eff, ratio.ap_eff, "effective tendon area"),
        (
            candidate.ap_eff_weighted,
            ratio.ap_eff_weighted,
            "bond-weighted tendon area",
        ),
        (candidate.rho_p_eff, ratio.rho_p_eff, "effective ratio"),
    ):
        _assert_2023_number(actual, expected, label)
    if candidate.xi1 != ratio.xi1_by_element[candidate.bar_index]:
        raise TraceValidationError("2023 candidate xi1 is stale")
    return ratio, direct_area
...
def _tendon_bond_steps_2023(
    ...
    aligned = _identity_steps(
        ...
        role=ROLE_COMPUTED,
        dependencies=(*dependencies, xi_input),
        prefix=prefix,
    )
    steps = [xi_input, *aligned]
...
def _bending_candidate_steps_2023(
    ...
    *,
    ...
    ratio_result = (
        _candidate_ratio_2023(candidate, replay, state, direct=False)
        if replay.blocks.tendons
        else None
    )
    ratio = None if ratio_result is None else ratio_result[0]
    ...
    weighted_step = value_step(
        ...
        candidate.ap_eff_weighted,
        SQUARE_METRE,
        TENDON_BOND_2023 if tendon_steps else EFFECTIVE_RATIO_2023,
        (*dependencies, ap_step, *tendon_steps),
        (
            "Formula (9.6) bond-weighted Ap contribution"
            if tendon_steps else "Bond-weighted Ap contribution"
        ),
    )
    rho_step = value_step(
    ...
    steps = [
        ...
        ap_step, *tendon_steps, weighted_step, rho_step, sigma_step,
        phi_step, cover_step,
...
def _direct_candidate_steps_2023(
    ...
    *,
    ...
    _assert_2023_number(candidate.k1_r, 1.0, "direct curvature factor")
    _assert_2023_number(candidate.kfl, 1.0, "direct flexural coefficient")
    ratio, direct_area = _candidate_ratio_2023(
    ...
    width_step = value_step(
        "rectangle-width", "Direct-tension rectangle width", direct_area.width,
        METRE, GEOMETRY, dependencies, "Resolved rectangle local width",
    )
    height_step = value_step(
        "rectangle-height", "Direct-tension rectangle height", direct_area.height,
        METRE, GEOMETRY, dependencies, "Resolved rectangle local height",
    )
    band_steps = {
        face: value_step(
            f"band-{face}",
            f"Figure 9.3 {face} effective band depth",
            getattr(direct_area, face),
            METRE,
            EFFECTIVE_AREA_2023,
            (*dependencies, x_step, y_step, area_step),
            "Retained face-specific Figure 9.3 band depth",
        )
        for face in ("left", "right", "bottom", "top")
    }
    inner_width = max(
        direct_area.width - direct_area.left - direct_area.right, 0.0
    )
    inner_height = max(
        direct_area.height - direct_area.bottom - direct_area.top, 0.0
    )
    inner_width_step = value_step(
        "inner-width", "Uncovered inner rectangle width", inner_width, METRE,
        EFFECTIVE_AREA_2023,
        (width_step, band_steps["left"], band_steps["right"]),
        "max(width - left - right, 0)",
    )
    inner_height_step = value_step(
        "inner-height", "Uncovered inner rectangle height", inner_height, METRE,
        EFFECTIVE_AREA_2023,
        (height_step, band_steps["bottom"], band_steps["top"]),
        "max(height - bottom - top, 0)",
    )
    hc_step = value_step(
        "hc-eff", "Direct-tension effective height", candidate.hc_ef, METRE,
        EFFECTIVE_AREA_2023,
        (band_steps["bottom"], band_steps["top"]),
        "max(bottom band, top band)",
    )
    bc_step = value_step(
        "bc-eff", "Direct-tension effective width", candidate.bc_ef, METRE,
        EFFECTIVE_AREA_2023,
        (band_steps["left"], band_steps["right"]),
        "max(left band, right band)",
    )
    recomposed_area = (
        direct_area.width * direct_area.height - inner_width * inner_height
    )
    _assert_2023_number(
        candidate.ac_eff, recomposed_area, "direct recomposed effective area"
    )
    ac_step = value_step(
        "ac-eff", "Direct-tension effective area", recomposed_area,
    ...
    tendon_steps = _tendon_bond_steps_2023(
        prefix,
        candidate,
        replay,
        ratio,
    ...
    _assert_2023_number(
        candidate.sr_max, spacing, "direct Formula (9.15) spacing"
    )
    spacing_step = value_step(
    ...
    steps = [
        position_step, x_step, y_step, area_step, width_step, height_step,
        *band_steps.values(), inner_width_step, inner_height_step,
        hc_step, bc_step, ac_step,
        as_step, ap_step, *tendon_steps, weighted_step, rho_step,
    ...
    _assert_2023_number(
        candidate.wk, calculated_width, "direct Formula (9.8) width"
    )
    width = value_step(
        "wk", "Candidate calculated crack width", calculated_width,
        MILLIMETRE, CRACK_WIDTH_2023,
