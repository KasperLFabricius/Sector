...
_ROUTE_IDENTITIES = {
    ...
    (False, "building-2023-bending"): _RouteIdentity(
        CODE_2023,
        EDITION_2023,
        METHOD_2023_APPLICABILITY,
        "building-2023",
...
def _reconstruct(
    ...
    is_2023 = route == "building-2023-bending"
    if is_2023:
...
def _route_steps(
    *,
    ...
    elif route == "building-2023-bending":
...
def _bending_candidate_steps_2023(
    ...
    *,
    ...
    """Expose the accepted 2023 refined-bending kernel without recomputing it."""
    ...
    hc_step = value_step(
        ...
        "EN 1992-1-1:2023 Figure 9.3 bending effective height",
    )
    ac_step = value_step(
        "ac-eff", "Effective tension area", candidate.ac_eff, SQUARE_METRE,
        EFFECTIVE_AREA_2023, (*dependencies, hc_step),
        "EN 1992-1-1:2023 Figure 9.3 bending effective area",
    )
    as_step = value_step(
    ...
    bond_step = value_step(
        "kb", "Bond factor", 0.9 if selected_k1 <= 1.0 else 1.2,
        ONE, BOND_2023, dependencies,
    ...
    spacing_step = value_step(
        ...
        (*dependencies, cover_step, phi_step, rho_step, flexural_step, bond_step),
        "Formula (9.15)",
    )
    steps = [
...
def _case_member(
    ...
    is_2023 = replay.route == "building-2023-bending"
...
def _aggregate_member(
    ...
    if finite_values:
        ...
        aggregate_reason = (
            "No retained 2023 crack-width case was assessed: "
            + " | ".join(not_assessed_reasons)
            if is_2023 and not_assessed_reasons
            else "No retained crack-width case is applicable."
        )
        final = _step(
            "crack-width-aggregate-result",
            "Governing retained crack width",
...
def _failed_member(
    ...
    if replay.route == "building-2023-bending":
