...
class RadialUtilResult:
    """Complete result from the existing applied-ray envelope selector."""
...
def radial_util_result(mx, my, ax, ay):
    ...
    Returns the utilisation, governing endpoint, demand radius, and intersected
    resistance radius. The governing endpoint is ``None`` when there is no applied
    direction (zero moment) or the ray misses the envelope.
    """
...
def radial_util(mx, my, ax, ay):
    """Backward-compatible ``(utilisation, governing_index)`` selector API."""
