...
class RadialUtilResult:
    """Retained result of one applied ray and plastic-envelope intersection."""
...
def radial_util_result(mx, my, ax, ay):
    ...
    The retained result includes the exact applied resultant, ray-intersection
    resistance, utilisation, and selected swept member. The resistance and member
    are ``None`` when there is no applied direction or the ray misses the envelope.
    """
...
def radial_util(mx, my, ax, ay):
    """Return the established ``(utilisation, governing index)`` public pair."""
