...
class RadialUtilResult:
    """Retained result of one applied ray and envelope intersection."""
...
def radial_util_result(mx, my, ax, ay):
    """Solve an applied ``(Mx, My)`` ray against the plastic M-M envelope.
    ...
    The retained result includes the exact demand, ray resistance, utilisation,
    and selected swept member. Resistance and member are ``None`` when the ray
    has no usable forward intersection.
    """
    a_rad = float(np.hypot(ax, ay))
    if a_rad < 1e-9:
        return RadialUtilResult(0.0, None, a_rad, None)
    ...
    if not hit.any():
        return RadialUtilResult(math.inf, None, a_rad, None)
    ...
    return RadialUtilResult(a_rad / cap, gov, a_rad, cap)


def radial_util(mx, my, ax, ay):
    """Return the established ``(utilisation, governing index)`` pair."""
