"""Frozen PR-08D.3a.1 successful CT-010a trace contract."""
...
from .calculation_trace import (
    ...
    RESULT_POSITIVE_INFINITY, RESULT_UNDEFINED, SOURCE_INPUT, SOURCE_PROJECT,
...
ALL_FINAL_STATES = frozenset({
    RESULT_FINITE, RESULT_POSITIVE_INFINITY, RESULT_NEGATIVE_INFINITY,
    RESULT_UNDEFINED, RESULT_FAILED,
})


def code_sources(
    edition: str, reinforcement_kind: str, *, custom_detail: bool = False,
):
    ...
    proof = TraceSource(
        SOURCE_STANDARD,
        ("en-1992-prestress-fatigue-yield" if tendon
         else "en-1992-reinforcement-fatigue-yield"),
        edition,
        SourceCitation(document, proof_clause,
...
def _member_contract(member: MemberSpec) -> TraceMemberContract:
    return TraceMemberContract(
        ...
        sources=frozenset(TraceSourceContract(
            node.source.kind, node.source.method_id, node.source.edition)
        ...
        step_metadata=tuple(TraceStepMetadataContract(
            node.step_id, node.role, node.source) for node in member.nodes))
...
def registry_for(members: tuple[MemberSpec, ...]) -> TraceRegistryContract:
    ...
    return TraceRegistryContract(
        REGISTRY_ID,
        (TraceFamilyContract(
            FAMILY_ID, tuple(_member_contract(member) for member in members)),))
