"""Frozen CT-010a reinforcement-fatigue trace structure."""
...
def description_step_id(index, name, description):
    return (
        f"{bin_prefix(index, name)}-description-"
...
def output_element_prefix(si, spectrum, ei, element_id):
    return (
        f"{spectrum_prefix(si, spectrum)}-e{ei:02d}-"
...
def _geometry_vector(plan, shape):
    leaves = []
    for ring_index, ring in enumerate(shape.blocks.geometry.rings):
        for point_index, _point in enumerate(ring):
            ...
            leaves.append(plan.add(
                f"{prefix}-x", "Concrete vertex x", M,
                ROLE_USER_INPUT, INPUT_SOURCE))
            leaves.append(plan.add(
                f"{prefix}-y", "Concrete vertex y", M,
                ROLE_USER_INPUT, INPUT_SOURCE))
...
def _material_vector(plan, shape):
    leaves = [plan.add(
    ...
    return plan.add(
        "material-vector", "Immutable material assignments", ONE,
        ROLE_COMPUTED, ADAPTER_SOURCE, *leaves)
...
def element_steps(shape):
    ...
    if shape.joint:
        entered.append(plan.add(
            "input-gamma-c", "Entered gamma_c for joint fatigue input",
            ONE, ROLE_USER_INPUT, INPUT_SOURCE))
    ...
    normalised = plan.add(
        ...
        ROLE_COMPUTED, ADAPTER_SOURCE, *entered, *resolved,
        _geometry_vector(plan, shape), _material_vector(plan, shape))
    ...
    for index, name in enumerate(shape.bin_names):
        ...
        long = plan.add(
            f"{prefix}-upstream-stress-long", "Raw long-term stress", MPA,
            ROLE_METHOD_VALUE, ENGINE_SOURCE)
        total = plan.add(
            f"{prefix}-upstream-stress-total", "Raw fatigue total stress", MPA,
            ROLE_METHOD_VALUE, ENGINE_SOURCE)
        elastic = plan.add(
            f"{prefix}-upstream-stress-total-elastic",
            "Raw uncorrected total stress", MPA,
            ROLE_METHOD_VALUE, ENGINE_SOURCE)
        design = plan.add(
            f"{prefix}-upstream-stress-total-design",
            "Raw design total stress", MPA,
            ROLE_METHOD_VALUE, ENGINE_SOURCE)
    ...
    damage = plan.add(
        "damage-total", "Element Miner damage", ONE,
...
def output_steps(shape):
    ...
    joint_inputs = []
    if shape.joint:
        joint_inputs.append(plan.add(
            "input-gamma-c", "Entered gamma_c for joint fatigue input", ONE,
            ROLE_USER_INPUT, INPUT_SOURCE))
    convergence, passed, governing, spectrum_utils = [], [], [], []
    ...
    all_converged = plan.add(
        ...
        steel, concrete, *joint_inputs, *convergence)
    all_passed = plan.add(
...
def invalid_steps(shape):
    ...
    errors = [
        plan.add(
            invalid_error_id(index, message),
