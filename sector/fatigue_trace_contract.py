"""Closed dependency registry for CT-010a reinforcement fatigue."""
...
class FamilyPlan:
    ...
    errors: tuple[str, ...]
    factors: tuple[float | int | None, float | int | None,
                   float | int | None] | None
...
def success_plan(*, leaves, rows, context, edition, concrete_method_type,
                 concrete_parameters_type, factors):
    ...
    for row in rows:
        ...
        assessments.append(AssessmentPlan(
            ...
            bins=tuple(BinPlan(*item) for item in bins),
            sn_source=sn_source,
...
def invalid_plan(*, leaves, errors, context, edition,
    ...
    return FamilyPlan(
        "invalid", tuple(leaves), (), token, None, axes, tuple(errors), None
    )


def _input_root(graph: _Graph, family: FamilyPlan):
    leaves = tuple(
    ...
    gamma_s = graph.add(
        "input-gamma-s", "Entered reinforcement partial factor", ONE,
        ROLE_USER_INPUT, INPUT_SOURCE,
    )
    gamma_ff = graph.add(
        "input-gamma-ff", "Entered fatigue action factor", ONE,
        ROLE_USER_INPUT, INPUT_SOURCE,
    )
    gamma_c = graph.add(
        "input-gamma-c", "Entered concrete sibling factor", ONE,
        ROLE_USER_INPUT, INPUT_SOURCE,
    )
    return graph.add(
...
def assessment_steps(family: FamilyPlan, plan: AssessmentPlan):
    ...
    for bin_plan in plan.bins:
        ...
        fatigue = graph.add(
            ...
            ROLE_COMPUTED, plan.bond_source, normal, long, elastic, converged,
        )
        design = graph.add(
        ...
        frange = graph.add(f"{bp}-fatigue-range", "Fatigue stress range", STRESS,
                           ROLE_COMPUTED, plan.bond_source, long, fatigue)
        ...
        exponent = graph.add(
            ...
            ROLE_COMPUTED, plan.sn_source, drange, reference, k1, k2,
        )
        loglife = graph.add(
...
def aggregate_steps(family: FamilyPlan):
    graph = _Graph()
    normal = _input_root(graph, family)
    summaries = []
