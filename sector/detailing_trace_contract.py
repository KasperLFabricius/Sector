...
def concrete_leaf_id(name: str) -> str:
    return f"material-concrete-{name.lower().replace('_', '-')}"
...
def expected_transverse_steps(shape: TransverseShape) -> tuple[StepSpec, ...]:
    ...
    material_leaves = [
        rows.add(concrete_leaf_id(name), f"Assigned concrete {name}",
    ...
    minimum = rows.add("minimum-ratio", "Minimum transverse ratio", ONE,
                       ...
                       concrete_leaf_id("fck"), "input-link-fywk", factor)
    adapter_steps = []
    for direction in shape.directions:
        c = direction.component
