...
class PlasticResultants:
    """Authoritative accumulated forces, moments, and extreme strains."""
...
@dataclass(frozen=True, slots=True)
class PlasticReconstruction:
    """Canonical strain plane and resultants at one retained compression depth."""

    curvature: float
    neutral_axis_depth: float
    resultants: PlasticResultants


@dataclass(frozen=True, slots=True)
class PlasticSweepPlan:
    """Canonical requested-to-solver neutral-axis sweep mapping."""
...
def plastic_sweep_plan(
    v_min: float,
    v_max: float,
    v_inc: float,
) -> PlasticSweepPlan:
    """Return the exact angle order consumed by the plastic solver.

    The requested increment is a maximum resolution. The normalised increment
    lands on the requested end angle without overshoot; a full turn omits its
    duplicate final point because the polygon closes back to the first member.
    """
    ...
    count = (
        int(round((solver_max - requested_min) / normalised_increment)) + 1
    )
    angles = tuple(
        requested_min + index * normalised_increment
        for index in range(count)
    )
    return PlasticSweepPlan(
        ...
        solver_increment=normalised_increment,
        closed=closed,
        angles=angles,
    )
...
def reconstruct_plastic_resultants(
    section: Section,
    concrete: Concrete,
    steel: MildSteel,
    V_deg: float,
    ...
    *,
    ...
    """Re-evaluate one retained strain plane through the solver accumulator.

    This is the authoritative reconstruction path for solver-owned evidence. It
    derives the governing curvature from the supplied angle and compression depth,
    then calls the same concrete/reinforcement accumulation kernel used by
    :func:`plastic_capacity_at_angle`. No search or alternative capacity method is
    introduced.
    """

    for label, value in (
        ("neutral-axis angle", V_deg),
        ("compression depth", compression_depth),
    ):
        if (
            type(value) not in {int, float}
            or type(value) is bool
            ...
            raise ValueError(f"{label} must be a finite non-Boolean number")
    c = float(compression_depth)
    if c <= 0.0:
        raise ValueError("compression depth must be positive")
    if type(n_bands) is not int or type(n_bands) is bool or n_bands <= 0:
    ...
    angle = math.radians(float(V_deg))
    direction_x, direction_y = math.cos(angle), math.sin(angle)
    bar_data = (
        prep.bx,
        prep.by,
        prep.ba,
        prep.bx * direction_x + prep.by * direction_y,
    )
    tendon_data = (
        prep.tx,
        prep.ty,
        prep.ta,
        prep.tx * direction_x + prep.ty * direction_y,
    )
    projections = (
        prep.verts[:, 0] * direction_x + prep.verts[:, 1] * direction_y
    )
    s_max = float(projections.max())
    curvature = _governing_curvature(
        bar_laws,
        tendon_laws,
        s_max,
        c,
    ...
    return PlasticReconstruction(
        curvature=curvature,
        neutral_axis_depth=s_max - c,
