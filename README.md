...
Current internal version: **Sector 0.96.1**. See [CHANGELOG.md](CHANGELOG.md) and
the [v0.96.1 release notes](docs/v0961_release_notes.md).
...
* **Elastic outputs and crack width** - concrete/reinforcement stresses and
  transformed properties. When an Elastic action requests crack width `wk`,
  independent user-specified long-term and short-term criteria apply. A
  criterion of 0 mm keeps that duration's crack width as a calculated output
  without a comparison; a positive criterion produces
  `WITHIN USER-SPECIFIED LIMIT` or `EXCEEDS USER-SPECIFIED LIMIT` with the
...
`BUILD.bat` creates `Sector-v0.96.1-windows-portable.zip` and a SHA-256 sidecar.
