...
def test_release_version_is_0_96_2():
    assert __version__ == "0.96.2"
...
def test_internal_version_declares_the_simple_unsigned_portable_build():
    ...
    release_notes = (ROOT / "docs" / "v0962_release_notes.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "Current internal version: **Sector 0.96.2**" in readme
    assert "[v0.96.2 release notes](docs/v0962_release_notes.md)" in readme
    assert "`BUILD.bat` creates `Sector-v0.96.2-windows-portable.zip`" in readme
    assert "## 0.96.2 - 2026-09-08" in changelog
    assert release_notes.startswith("# Sector 0.96.2 notes\n")
    assert "Supported project schema: 27" in release_notes
    assert "Sector-v0.96.2-windows-portable" in release_notes
    ...
    for live_surface in (product_identity, project_io_source):
        ...
        assert "Sector 0.96.2" in live_surface
