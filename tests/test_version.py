...
def test_release_version_is_0_94():
...
def test_internal_version_declares_the_simple_unsigned_portable_build():
    ...
    release_notes = (ROOT / "docs" / "v094_release_notes.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "Current internal version: **Sector 0.94**" in readme
    assert "[v0.94 release notes](docs/v094_release_notes.md)" in readme
    assert "`BUILD.bat` creates `Sector-v0.94-windows-portable.zip`" in readme
    assert "## 0.94 - 2026-08-14" in changelog
    assert release_notes.startswith("# Sector 0.94 notes\n")
    assert "Supported project schema: 25" in release_notes
    assert "Sector-v0.94-windows-portable" in release_notes
    ...
    for live_surface in (product_identity, project_io_source):
        ...
        assert "Sector 0.94" in live_surface
