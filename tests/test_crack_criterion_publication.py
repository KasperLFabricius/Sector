...
def test_crack_criteria_publication_retains_both_duration_limits_when_inactive() -> None:
    ...
    assert all(item.calculation_applied is False for item in publication)
...
def test_crack_criteria_publication_enables_only_positive_active_comparisons() -> None:
    publication = ordinary_crack_criteria_publication(
        _criteria(long_term=-0.0, short_term=0.25),
    ...
    assert long_term.calculation_applied is True
    assert long_term.comparison_enabled is False
    assert short_term.calculation_applied is True
...
@pytest.mark.parametrize(
    "value",
    [None, True, False, "0.30", b"0.30", -0.01, math.nan, math.inf, -math.inf],
)
