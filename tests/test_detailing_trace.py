...
def test_biaxial_member_final_aggregates_component_outcomes():
    # P1-A: retained biaxial consumers read only the per-component screens
    # (the view suppresses the torsion-only top screen beside directional
    # interactions), so the member final aggregates them worst-first:
    # any FAIL -> 0.0, else any applicable PASS -> 1.0, else 4.0.
    shear_common = {"res": {"valid": True, "vrd_c": 200.0}, "v_ed": 30.0,
                    "model_2023": False}

    def _face(tension_low, trd_c, *, res_valid=True):
        face_shear = {"res": {"valid": res_valid, "vrd_c": 200.0},
                      "v_ed": 30.0, "model_2023": False}
        torsion = {"subdivided": False, "t_ed": 15.0,
                   "primary": {"trd_c": trd_c}}
        torsion["min_reinf"] = _refreshed_screen(
            {"torsion": torsion, "shear": face_shear if res_valid else None})
        return {"tension_low": tension_low, "shear": face_shear,
                "torsion": torsion}

    def _crafted_biaxial(component_faces):
        directions, interactions = {}, {}
        for component, faces in component_faces.items():
            container = _decorate_directional(dict(
                shear_common, axis="y" if component == "vx" else "x",
                face_mode="auto", associated_moment=0.0,
                face_candidates=faces))
            directions[component] = container
            states = [_face_state(row) for row in faces]
            governing = max(range(len(states)),
                            key=lambda i: (_PRIORITY[states[i][0]],
                                           states[i][1]))
            aggregate = next(
                status for status in
                ("INVALID", "FAIL", "NOT ASSESSED", "NOT RUN", "PASS")
                if status in {state for state, _ in states})
            gov_row = faces[governing]
            gov_face = "negative" if gov_row["tension_low"] else "positive"
            entry = dict(gov_row["torsion"])
            entry["min_reinf"] = dict(entry["min_reinf"],
                                      directional_status=aggregate,
                                      governing_face=gov_face)
            entry["directional_min_reinf_status"] = aggregate
            entry["directional_min_reinf_governing_face"] = gov_face
            interactions[component] = entry
        top = {"subdivided": False,
               "min_reinf": {"applicable": False, "reason": "no shear check"},
               "directional_interactions": interactions}
        return {"shear": {"directions": directions}, "torsion": top}

    inp = _input(shear_on=True, torsion_on=True, torsion_T=15.0,
                 shear_V=30.0, shear_Vx=20.0, shear_Vy=30.0)
    failing = _crafted_biaxial({
        "vx": [_face(True, 10.0), _face(False, 60.0)],   # governing FAIL
        "vy": [_face(True, 25.0), _face(False, 60.0)],   # governing PASS
    })
    steps = _steps(_bundle(inp, failing), "min-reinf-screen")
    assert steps["screen-vx-status"].result.value == 0.0
    assert steps["screen-primary-status"].result.value == 4.0
    assert steps["ct-008-min-reinf-screen-result"].result.value == 0.0

    mixed = _crafted_biaxial({
        "vx": [_face(True, 25.0), _face(False, 60.0)],           # PASS
        "vy": [_face(True, 60.0, res_valid=False),
               _face(False, 60.0, res_valid=False)],             # NA screens
    })
    steps = _steps(_bundle(inp, mixed), "min-reinf-screen")
    assert steps["screen-vy-status"].result.value == 4.0
    assert steps["ct-008-min-reinf-screen-result"].result.value == 1.0

    # Real biaxial run: the conservative NOT ASSESSED-governing components
    # publish not-applicable screens; the member final stays 4.0.
    real_out = _candidate(inp)
    real_steps = _steps(_bundle(inp, real_out), "min-reinf-screen")
    assert real_steps["ct-008-min-reinf-screen-result"].result.value == 4.0


def test_face_labels_require_exact_booleans():
    # P1-B: tension_low must be present with exact Boolean type; missing,
    # null and 0/1 integer lookalikes are rejected on resealed rows.
    single = _input(shear_on=True, torsion_on=True, torsion_T=15.0,
                    shear_V=40.0, shear_Vx=40.0, shear_Vy=0.0)
    single_out = _candidate(single)
    single_bundle = _bundle(single, single_out)
    genuine = single_out["shear"]["face_candidates"][0]["tension_low"]
    assert type(genuine) is bool

    def _tampered(mutate):
        changed = copy.deepcopy(single_out)
        mutate(changed["shear"]["face_candidates"][0])
        return changed

    variants = (
        _tampered(lambda row: row.pop("tension_low")),
        _tampered(lambda row: row.update(tension_low=None)),
        _tampered(lambda row: row.update(tension_low=int(genuine))),
        _tampered(lambda row: row.update(tension_low=1 - int(genuine))),
    )
    ...
    biaxial_bundle = _bundle(biaxial, biaxial_out)
    changed = copy.deepcopy(biaxial_out)
    row = changed["shear"]["directions"]["vy"]["face_candidates"][0]
    row["tension_low"] = int(row["tension_low"])
    with pytest.raises(TraceValidationError):
        validate_detailing_trace_family(
            biaxial_bundle, biaxial, changed, input_sha256=INPUT_SHA,
            result_sha256=RESULT_SHA, context=CONTEXT)
...
def test_proportional_operand_reseal_of_non_governing_faces_rejected():
    # Generalization 1: every face binds the full operand set, so scaling a
    # non-governing face's t_ed and trd_c together (value preserved, payload
    # refreshed) changes the sealed bundle and is rejected.
    shear_common = {"res": {"valid": True, "vrd_c": 200.0}, "v_ed": 30.0,
                    "model_2023": False, "axis": "y", "face_mode": "auto",
                    "associated_moment": 0.0}
    ...
    def _face(tension_low, trd_c):
        torsion = {"subdivided": False, "t_ed": 15.0,
                   "primary": {"trd_c": trd_c}}
        torsion["min_reinf"] = _refreshed_screen(
            {"torsion": torsion, "shear": shear_common})
        return {"tension_low": tension_low, "shear": dict(shear_common),
                "torsion": torsion}

    def _scaled(out, *path):
        changed = copy.deepcopy(out)
        row = changed
        for key in path:
            row = row[key]
        row["torsion"]["t_ed"] *= 2.0
        row["torsion"]["primary"]["trd_c"] *= 2.0
        row["torsion"]["min_reinf"] = _refreshed_screen(
            {"torsion": row["torsion"], "shear": row["shear"]})
        return changed

    single = _input(shear_on=True, torsion_on=True, torsion_T=15.0,
                    shear_V=30.0, shear_Vx=30.0, shear_Vy=0.0)
    face_b, face_a = _face(True, 25.0), _face(False, 60.0)
    top_torsion = dict(face_b["torsion"])
    top_torsion["min_reinf"] = dict(face_b["torsion"]["min_reinf"],
                                    directional_status="PASS",
                                    governing_face="negative")
    ...
    single_out = {
        "shear": _decorate_directional(
            dict(shear_common, face_candidates=[face_b, face_a])),
        "torsion": top_torsion,
    }
    bundle = _bundle(single, single_out)
    steps = _steps(bundle, "min-reinf-screen")
    assert steps["screen-primary-governing-face"].result.value == 0.0
    assert steps["upstream-screen-primary-face-01-t-ed"].result.value == 15.0
    baseline = bundle.to_dict()
    scaled = _scaled(single_out, "shear", "face_candidates", 1)
    scaled_screen = scaled["shear"]["face_candidates"][1]["torsion"][
        "min_reinf"]
    assert scaled_screen["value"] == pytest.approx(
        single_out["shear"]["face_candidates"][1]["torsion"]["min_reinf"][
            "value"])
    rebuilt = _bundle(single, scaled)
    assert rebuilt.to_dict() != baseline
    with pytest.raises(TraceValidationError):
        validate_detailing_trace_family(
            bundle, single, scaled, input_sha256=INPUT_SHA,
            result_sha256=RESULT_SHA, context=CONTEXT)

    # Biaxial: same reseal on a non-governing face of a component.
    biaxial = _input(shear_on=True, torsion_on=True, torsion_T=15.0,
                     shear_V=30.0, shear_Vx=20.0, shear_Vy=30.0)
    directions, interactions = {}, {}
    for component in ("vx", "vy"):
        faces = [_face(True, 25.0), _face(False, 60.0)]
        container = _decorate_directional(dict(
            shear_common, axis="y" if component == "vx" else "x",
            face_candidates=faces))
        directions[component] = container
        entry = dict(faces[0]["torsion"])
        entry["min_reinf"] = dict(entry["min_reinf"],
                                  directional_status="PASS",
                                  governing_face="negative")
        entry["directional_min_reinf_status"] = "PASS"
        entry["directional_min_reinf_governing_face"] = "negative"
        interactions[component] = entry
    biaxial_out = {
        "shear": {"directions": directions},
        "torsion": {"subdivided": False,
                    "min_reinf": {"applicable": False,
                                  "reason": "no shear check"},
                    "directional_interactions": interactions},
    }
    biaxial_bundle = _bundle(biaxial, biaxial_out)
    scaled = _scaled(biaxial_out, "shear", "directions", "vy",
                     "face_candidates", 1)
    assert _bundle(biaxial, scaled).to_dict() != biaxial_bundle.to_dict()
    with pytest.raises(TraceValidationError):
        validate_detailing_trace_family(
            biaxial_bundle, biaxial, scaled, input_sha256=INPUT_SHA,
            result_sha256=RESULT_SHA, context=CONTEXT)


def _boolean_targets(out, *, uniaxial=False):
    # Enumerate exactly the Boolean fields the screen member READS in the
    # given mode; fields retained itself never reads on a branch (e.g. a
    # not-applicable face's shear model_2023) are not reader surface.
    targets = []
    top = out["torsion"].get("min_reinf") or {}
    targets.append(["torsion", "min_reinf", "applicable"])
    for key in ("ok", "solid", "model_2023"):
        if key in top:
            targets.append(["torsion", "min_reinf", key])
    if uniaxial:
        targets.append(["shear", "res", "valid"])
        if "model_2023" in out["shear"]:
            targets.append(["shear", "model_2023"])
        if "subdivided" in out["torsion"]:
            targets.append(["torsion", "subdivided"])
        return targets
    # Single-active: the container res aliases the governing face's res in
    # retained output, so the container mutation is exercised via the rows.
    for index, row in enumerate(out["shear"]["face_candidates"]):
        base = ["shear", "face_candidates", index]
        targets.append(base + ["tension_low"])
        check = (row.get("torsion") or {}).get("min_reinf") or {}
        if isinstance(row.get("shear"), dict):
            targets.append(base + ["shear", "res", "valid"])
            if check.get("applicable") and "model_2023" in row["shear"]:
                targets.append(base + ["shear", "model_2023"])
        torsion = row.get("torsion")
        if isinstance(torsion, dict):
            if "subdivided" in torsion:
                targets.append(base + ["torsion", "subdivided"])
            targets.append(base + ["torsion", "min_reinf", "applicable"])
            for key in ("ok", "solid", "model_2023"):
                if key in check:
                    targets.append(base + ["torsion", "min_reinf", key])
    return targets


def test_every_boolean_screen_field_rejects_integer_lookalikes():
    # Generalization 2: exact-Boolean typing across the whole upstream
    # reader surface; truthiness-preserving 0/1 lookalikes fail closed.
    cases = (
        (_input(shear_on=True, torsion_on=True, shear_V=30.0,
                torsion_T=15.0), True),
        (_input(shear_on=True, torsion_on=True, torsion_T=15.0,
                shear_V=40.0, shear_Vx=40.0, shear_Vy=0.0), False),
    )
    for inp, uniaxial in cases:
        out = _candidate(inp)
        bundle = _bundle(inp, out)
        targets = _boolean_targets(out, uniaxial=uniaxial)
        assert len(targets) >= (5 if uniaxial else 8)
        for path in targets:
            changed = copy.deepcopy(out)
            node = changed
            for key in path[:-1]:
