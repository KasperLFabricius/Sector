...
def test_screen_directional_single_biaxial_and_ordering_oracle():
    ...
    crafted_out = {
        "shear": dict(shear_face, face_candidates=[face_a, face_b]),
        "torsion": top_torsion,
    }
    steps = _steps(_bundle(crafted, crafted_out), "min-reinf-screen")
    assert steps["screen-primary-governing-face"].result.value == 1.0
    ...
    top_torsion["directional_min_reinf_governing_face"] = "negative"
    mixed_out = {
        "shear": dict(shear_face, face_candidates=[face_na, face_fail]),
        "torsion": top_torsion,
    }
...
def test_screen_face_states_legends_reorder_and_governing_tampers():
    ...
    top_torsion["directional_min_reinf_status"] = "FAIL"
    top_torsion["directional_min_reinf_governing_face"] = "negative"
    out = {"shear": dict(shear_face, face_candidates=[not_run, face_fail]),
           "torsion": top_torsion}
    bundle = _bundle(crafted, out)
    steps = _steps(bundle, "min-reinf-screen")
    assert steps["upstream-screen-primary-face-00-run"].result.value == 0.0
    assert steps["screen-primary-face-00-assessment-status"].result.value == (
    ...
    invalid_out = {
        "shear": dict(shear_face,
                      face_candidates=[invalid_face, _face(True, 60.0)]),
        "torsion": invalid_top,
    }
    with pytest.raises(TraceValidationError):
        build_detailing_trace_family(
...
def test_non_governing_face_screens_are_fully_validated():
    ...
    top_torsion["directional_min_reinf_status"] = "PASS"
    top_torsion["directional_min_reinf_governing_face"] = "negative"
    out = {"shear": dict(shear_face, face_candidates=[face_a, face_b]),
           "torsion": top_torsion}
    bundle = _bundle(crafted, out)
    steps = _steps(bundle, "min-reinf-screen")
    assert steps["screen-primary-governing-face"].result.value == 1.0
    ...
    for key, replacement in mutations:
        ...
        changed["shear"]["face_candidates"][0]["torsion"]["min_reinf"][
            key] = replacement
        with pytest.raises(TraceValidationError):
            validate_detailing_trace_family(
    ...
    face_screen = out["shear"]["face_candidates"][0]["torsion"]["min_reinf"]
    for kind, path, detail in list(_walk(face_screen)):
        ...
        for variant in variants:
            changed = copy.deepcopy(out)
            changed["shear"]["face_candidates"][0]["torsion"][
                "min_reinf"] = variant
            with pytest.raises(TraceValidationError):
                validate_detailing_trace_family(
                    bundle, crafted, changed, input_sha256=INPUT_SHA,
