...
def test_crack_limit_owner_decision_is_exact_without_inferred_basis() -> None:
    decision = _fixture()["crack_limit_decision"]
    assert decision == {
        "owning_pr": "PR-A04",
        "project_schema_after_slice": 26,
        "long_term_key": "sls_crack_limit_long_mm",
        "short_term_key": "sls_crack_limit_short_mm",
        "positive_finite_compares_matching_duration_only": True,
        "exact_numeric_zero_means_calculated_comparison_not_assessed": True,
        "cross_duration_maximum": False,
        "cross_duration_fallback": False,
        "standard_limit_inferred": False,
        "schema_25_positive_single_limit_migration": "copy to both values",
        "schema_25_blank_limit_migration": "two numeric zeros",
        "heightened_formula_uses_separate_positive_operand": True,
        "ordinary_zero_passed_to_heightened_formula": False,
    }
    programme = _text(PROGRAMME)
    assert "compares only its matching calculated duration" in programme
    assert "exact numeric zero means the width is calculated and disclosed" in programme
    assert "no cross-duration maximum or\n  inferred standard limit" in programme


def test_fatigue_and_overview_matrices_are_owned_but_not_fabricated_here() -> None:
    contracts = _fixture()["deferred_acceptance_contracts"]
    assert set(contracts) == {"PR-A05", "PR-A06"}
    ...
    programme = _text(PROGRAMME)
    assert "must be frozen there before\nimplementation" in programme
    assert "PR-A00 intentionally\ncontains neither matrix" in programme
