...
def _candidate_output(inp, *, converged_override=None):
    ...
    elastic = dict(
        ...
        long=[s / 1000.0 for s in r.bar_stress_long],
        dif=[s / 1000.0 for s in r.bar_stress_dif],
        rst1=[s / 1000.0 for s in r.bar_stress_rst1],
        converged=converged,
...
def test_frozen_plane_units_and_dependency_edges():
    ...
    # Ac,eff clips the section rings: geometry is a direct operand.
    assert edges("fine-ac-eff") == ("normalised-crack-inputs", "fine-hc-ef")
    ...
    # Source truthfulness: the long-case cracked state is unpublished, so
    # its leaves are cross-checked PROJECT REPLAY; the short-case leaves are
    # genuine published CT-005 evidence and carry the published values.
    from sector.crack_trace_contract import REPLAY_SOURCE, UPSTREAM_SOURCE

    assert UPSTREAM_SOURCE.method_id == "sector-ct-005-elastic-evidence"
    assert REPLAY_SOURCE.method_id == "sector-crack-cross-checked-replay"
    assert steps["upstream-plane-eps0"].source == REPLAY_SOURCE
    assert steps[stress_leaves[0]].source == REPLAY_SOURCE
    short_steps = _member_steps(bundle, "-short-term")
    assert short_steps["upstream-plane-eps0"].source == UPSTREAM_SOURCE
    assert short_steps[stress_leaves[0]].source == UPSTREAM_SOURCE
    for name, value in zip(("eps0", "kx", "ky"),
                           out["elastic"]["stress_plane"]):
        assert short_steps[f"upstream-plane-{name}"].result.value == float(
            value)
    for index, leaf in enumerate(stress_leaves):
        assert short_steps[leaf].result.value == float(
            out["elastic"]["total"][index])


def test_holed_section_effective_area_clips_the_rings():
    # Ac,eff is a ring-clipping quantity: on a holed section it is NOT
    # b * hc,ef. The hole sits fully inside the tension band (and the
    # cracked compression zone stays hole-free, so the closed-form neutral
    # axis still applies).
    hole = [(0.10, 0.03), (0.20, 0.03), (0.20, 0.11), (0.10, 0.11)]
    section = Section.from_polygon(
        corners=[(0.0, 0.0), (0.3, 0.0), (0.3, 0.6), (0.0, 0.6)],
        bars_xy_area_mm2=[(0.05, 0.05, 750.0), (0.25, 0.05, 750.0)],
        holes=[hole])
    inp = _beam_input(
        section=section,
        bar_elements=[
        ...
        Mx_el_l=150.0, Mx_el_s=0.0)
    out = _candidate_output(inp)
    assert out["elastic"]["cracked"] is True
    bundle = _bundle(inp, out)
    steps = _member_steps(bundle, "-long-term")
    as_total = 1500.0e-6
    x = _cracked_depth(0.3, 0.55, 6.0 * as_total)
    hc = min(2.5 * (0.6 - 0.55), (0.6 - x) / 3.0, 0.3)
    assert hc == pytest.approx(0.125)  # the 2.5(h-d) term governs here
    ac = 0.3 * hc - 0.10 * (min(hc, 0.11) - 0.03)
    assert steps["fine-hc-ef"].result.value == pytest.approx(hc, rel=1e-6)
    assert steps["fine-ac-eff"].result.value == pytest.approx(ac, rel=1e-4)
    assert steps["fine-ac-eff"].result.value < 0.3 * hc  # the hole matters
    assert steps["fine-rho-p-eff"].result.value == pytest.approx(
        as_total / ac, rel=1e-4)
    assert _validate(bundle, inp, out) is not None
...
def test_tendon_identity_bond_and_prestress_removed_short_stress():
    ...
    # The short-case upstream leaves are the PUBLISHED CT-005 totals; the
    # locked-in prestress strip happens in the sigma-s step, whose declared
    # tendon operands are the assigned modulus and initial strain.
    ...
    assert upstream == pytest.approx(sorted(float(v) for v in total),
                                     rel=1e-9)
    tendon_prefix = next(
        s[len("upstream-"):-len("-stress")] for s in steps
        if s.startswith("upstream-element-02-") and s.endswith("-stress"))
    assert steps[f"{tendon_prefix}-is"].result.value == pytest.approx(0.0005)
    sigma = steps.get("fine-candidate-02-sigma-s")
    if sigma is not None:  # tendon in the tension band on this fixture
        assert tuple(dep.step_id for dep in sigma.dependencies) == (
            f"upstream-{tendon_prefix}-stress", f"{tendon_prefix}-es",
            f"{tendon_prefix}-is")
        assert sigma.result.value == pytest.approx(
            float(total[2]) - locked_mpa, rel=1e-9)
...
def test_consumed_upstream_leaves_reject_mutation():
    ...
    mutations = (
        ...
        lambda o: o["elastic"]["long"].__setitem__(
            0, o["elastic"]["long"][0] + 2.0),
        lambda o: o["elastic"]["dif"].__setitem__(
            1, o["elastic"]["dif"][1] - 2.0),
        lambda o: o["elastic"].__setitem__(
            "rst1", o["elastic"]["rst1"][:-1]),
        lambda o: o["elastic"].__setitem__(
            "sigma_ct", o["elastic"]["sigma_ct"] + 0.3),
    )
...
def test_divergent_solver_rerun_fails_closed(monkeypatch):
    # If the retained published evidence and the currently imported solver
    # disagree, the cross-check must fail closed instead of sealing freshly
    # recomputed values under any label.
    import sector.crack_trace as module

    inp = _beam_input()
    out = _candidate_output(inp)  # published with the unpatched solver
    assert _build(inp, out) is not None
    original = module.solve_elastic_combined

    def perturbed_plane(*args, **kwargs):
        result = original(*args, **kwargs)
        shifted = dataclasses.replace(
            result.short_term, eps0=result.short_term.eps0 + 1.0e-3)
        return dataclasses.replace(result, short_term=shifted)

    monkeypatch.setattr(module, "solve_elastic_combined", perturbed_plane)
    with pytest.raises(TraceValidationError, match="published"):
        _build(inp, out)

    def perturbed_stress(*args, **kwargs):
        result = original(*args, **kwargs)
        stresses = list(result.bar_stress_total)
        stresses[0] = stresses[0] + 1.0
        return dataclasses.replace(result, bar_stress_total=stresses)

    monkeypatch.setattr(module, "solve_elastic_combined", perturbed_stress)
    with pytest.raises(TraceValidationError, match="published"):
        _build(inp, out)
