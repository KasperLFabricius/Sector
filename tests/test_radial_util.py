...
def test_util_at_a_vertex_is_the_vertex_radius():
    ...
    # (chord and radius coincide there).
    f = _radial_util()
    mx, my = _diamond(100.0)
    assert f(mx, my, 50.0, 0.0) == pytest.approx(0.5)     # along +Mx -> (100, 0) vertex
    assert f(mx, my, 0.0, 80.0) == pytest.approx(0.8)     # along +My -> (0, 100) vertex
...
def test_util_follows_the_chord_not_the_radius():
    ...
    assert f(mx, my, 35.0, 35.0) == pytest.approx(35.0 * np.sqrt(2.0) / cap)
    assert f(mx, my, 50.0, 50.0) == pytest.approx(1.0)    # exactly on the chord
    assert f(mx, my, 100.0, 100.0) == pytest.approx(2.0)  # twice the chord distance
...
def test_util_matches_dense_circle_radius():
    ...
    assert f(mx, my, 50.0, 0.0) == pytest.approx(0.5)     # +Mx is a sample point
    assert f(mx, my, 0.0, 80.0) == pytest.approx(0.8)     # +My is a sample point
