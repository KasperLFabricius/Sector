...
def test_manual_distinguishes_slab_density_from_finite_face_spacing():
    ...
    assert "reinforcement area per metre" in text
    assert r"A_s = A_{bar}\,1000/s" in text
...
def test_manual_documents_grouped_fatigue_inputs_results_and_methodology():
    ...
    for expected in (
        ...
        "N_{R,i}",
        r"D=\\sum_i",
        "Simplified reinforcement stress-range screen",
...
def test_manual_documents_modelled_direction_and_link_detailing_scope():
    ...
    for expected in (
        ...
        "12.2(2)",
        "initial closed 15-degree",
        "retained utilisation interval",
        "reported **Not assessed**",
        "D_{upper}",
        "\\rho_{w,min}",
        "0.063",
...
def test_manual_separates_nominal_shear_resistance_from_link_detailing():
    ...
    assert "including when links are entered" in structured_text
    assert "the same selected" in structured_text
    assert "gamma_V" in structured_text
