...
from sector.calculation_trace import (  # noqa: E402
    TraceBundle, TraceValidationError, seal_bundle,
)
...
from sector.fatigue_trace_contract import (  # noqa: E402
    INVALID_KEYS, SUCCESS_KEYS, code_sources,
)
...
def _input(*, steel: bool = True, concrete: bool = False):
    inp = _base(fatigue_check_steel=steel,
...
def _replay(inp, authoritative):
    ...
    return mock.patch.object(
        fatigue_analysis, "run_analysis", return_value=authoritative)
...
def _section_leaf(bundle):
    return next(
        step.step_id for step in bundle.calculations[0].steps
...
@pytest.mark.parametrize(
    "wrapper", [collections.OrderedDict, collections.UserDict])
...
def test_assessment_stress_is_derived_from_matched_state(fast_case):
    ...
    row = dataclasses.replace(
        row, stress_long_mpa=row.stress_long_mpa + 1.0,
    ...
    candidate["spectra"] = (dataclasses.replace(
        spectrum,
...
@pytest.mark.parametrize(
    "key", ["fatigue_on", "fatigue_check_steel", "fatigue_check_concrete"])
@pytest.mark.parametrize("lookalike", [0, 1])
def test_all_boolean_controls_reject_integer_lookalikes_before_success(
    fast_case, key, lookalike,
):
    inp, out, _bundle = fast_case
    changed = copy.deepcopy(inp)
    changed[key] = lookalike
    with pytest.raises(TraceValidationError, match=key):
        _build(changed, out, authoritative=out["fatigue"])


@pytest.mark.parametrize(
    "key", ["fatigue_on", "fatigue_check_steel", "fatigue_check_concrete"])
def test_all_boolean_controls_are_required_before_dispatch(key):
    inp = _input()
    inp.pop(key)
    with pytest.raises(TraceValidationError, match=key):
        _build(inp, {"fatigue": None})


def test_boolean_controls_are_checked_before_invalid_payload():
    inp = _input()
    invalid = fatigue_analysis.invalid_result(inp, errors=("external",))
    inp["fatigue_check_concrete"] = 0
    with pytest.raises(TraceValidationError, match="fatigue_check_concrete"):
...
@pytest.mark.parametrize(
    "edition,mild_clause,tendon_clause",
    [
        ("DS/EN 1992-1-1:2023", "5.2.4", "5.3.3"),
        ("DS/EN 1992-1-1:2005+A1:2014", "3.2", "3.3.6"),
    ],
)
def test_proof_source_is_selected_by_edition_and_reinforcement_kind(
    edition, mild_clause, tendon_clause,
):
    _sn_mild, mild = code_sources(edition, "mild")
    _sn_tendon, tendon = code_sources(edition, "prestress")
    assert mild_clause in mild.citation.clause
    assert tendon_clause in tendon.citation.clause
    assert mild.method_id != tendon.method_id


def test_member_proof_nodes_use_kind_correct_sources(fast_case):
    _inp, _out, bundle = fast_case
    sources = {}
    for calculation in bundle.calculations[:-1]:
        axes = {axis.name: axis.value for axis in calculation.axes}
        proof = next(
            step.source for step in calculation.steps
            if step.step_id.endswith("-yield-limit"))
        sources[axes["reinforcement_kind"]] = proof
    assert "5.2.4" in sources["mild"].citation.clause
    assert "5.3.3" in sources["prestress"].citation.clause
    assert sources["mild"].method_id != sources["prestress"].method_id
...
def test_geometry_position_changes_normalised_identity(fast_case):
    ...
    changed["section"] = Section(
        section.concrete, [moved, *section.bars[1:]], section.tendons)
...
def test_geometry_scalar_type_changes_section_leaf_without_numeric_change(
    ...
    changed["section"] = Section(
        section.concrete, [integer_x, *section.bars[1:]], section.tendons)
    assert type(changed["section"].bars[0].x) is int
    changed_bundle = _build(
        changed, out, authoritative=out["fatigue"])
...
def test_excluded_concrete_top_sibling_type_is_pinned(concrete_case, key):
...
def test_excluded_concrete_result_values_are_inert(concrete_case):
    ...
    result = dataclasses.replace(
        result, **{numeric: getattr(result, numeric) + 0.25})
    candidate["spectra"] = (dataclasses.replace(
        spectrum, concrete=(result, *spectrum.concrete[1:])),)
    assert _build(inp, {"fatigue": candidate}).to_dict() == bundle.to_dict()
...
def test_excluded_concrete_result_container_type_is_pinned(concrete_case):
    ...
    spectrum = candidate["spectra"][0]
    candidate["spectra"] = (dataclasses.replace(
        spectrum, concrete=list(spectrum.concrete)),)
    with pytest.raises(TraceValidationError, match="retained type"):
        _build(inp, {"fatigue": candidate})
