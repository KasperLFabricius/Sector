...
from sector.calculation_trace import (  # noqa: E402
    RESULT_FAILED, RESULT_POSITIVE_INFINITY, TraceValidationError, seal_bundle,
)
...
def test_invalid_payload_accepts_missing_boolean_flag():
    inp = _steel_input()
    inp.pop("fatigue_check_steel")
...
def test_invalid_payload_accepts_malformed_boolean_flag():
    inp = _steel_input(fatigue_check_concrete="not-a-boolean")
    out = fatigue_analysis.invalid_result(inp)
    bundle = build_fatigue_trace_family(
        inp, out, input_sha256=INPUT_HASH, result_sha256=RESULT_HASH
    )
