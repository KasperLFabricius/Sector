...
def _combined_components(*, links=False):
    ...
    if links:
        ...
        torsion_asw_over_s = math.pi * 10.0**2 / 4.0 / 150.0
        primary.update(trd_s=100.0, trd_max=80.0)
        interaction = {
            "valid": True,
            "cot": 1.5,
...
def _complete_active_transverse_components():
    ...
    out["shear"]["links"].update(_complete_torsion_chord_links())
    out["torsion"].update(
        t_ed=10.0,
        primary={
            "t_ed": 10.0,
...
def test_combined_prerequisites_bind_active_interaction_and_shear_shift_to_sources():
    ...
    stale_input = dict(inp, shear_link_dia=12.0)
    stale_input_assessment = capacity.evaluate_combined_prerequisites(
        stale_input,
        control,
    )
    assert stale_input_assessment.valid is False
    assert stale_input_assessment.torsion_valid is False
    assert (
        "torsion stirrup result is inconsistent with input geometry"
        in stale_input_assessment.reasons
    )

    stale_retained = _complete_active_transverse_components()
    stale_retained["torsion"]["asw_over_s"] *= 2.0
    assert capacity.evaluate_combined_prerequisites(
        inp,
        stale_retained,
    ).torsion_valid is False

    zero_suppression = _complete_active_transverse_components()
    zero_suppression["torsion"]["asw_over_s"] = 0.0
    zero_suppression["torsion"]["interaction"] = None
    zero_assessment = capacity.evaluate_combined_prerequisites(
        inp,
        zero_suppression,
    )
    assert zero_assessment.valid is False
    assert "shared strut-interaction evidence is malformed" in zero_assessment.reasons
    ...
    dead_torsion["shear"]["links"]["chord_candidates"] = [
        _chord_candidate(
            "shear_axis",
            "x",
            True,
...
def test_final_prerequisites_require_current_common_angle_only_for_live_actions():
    ...
    del preliminary["shear"]["links"]["chord_candidates"]
    preliminary["torsion"]["member_angle_selection"] = None
    del preliminary["torsion"]["interaction"]
    ...
    all_dead["torsion"]["interaction"].update(v_ed=0.0, value=0.0)
    assert capacity.evaluate_combined_prerequisites(inp, all_dead).valid

    missing_dead_interaction = copy.deepcopy(all_dead)
    del missing_dead_interaction["torsion"]["interaction"]
    assert capacity.evaluate_combined_prerequisites(
        inp, missing_dead_interaction
    ).valid is False

    null_dead_interaction = copy.deepcopy(all_dead)
    null_dead_interaction["torsion"]["interaction"] = None
    assert capacity.evaluate_combined_prerequisites(
        inp, null_dead_interaction
    ).valid is False
...
def test_torsion_chord_asl_binding_allows_only_subdivided_top_level_sum():
    ...
    legacy_only = copy.deepcopy(control)
    del legacy_only["shear"]["links"]["chord_candidates"]
    legacy_only["shear"]["links"]["chord"] = {"valid": True}
    legacy_assessment = capacity.evaluate_combined_prerequisites(inp, legacy_only)
    assert legacy_assessment.valid is False
    assert legacy_assessment.links_valid is False

    missing_face = copy.deepcopy(control)
    missing_face["shear"]["links"]["chord_candidates"].pop()
    missing_face_assessment = capacity.evaluate_combined_prerequisites(
        inp,
        missing_face,
    )
    assert missing_face_assessment.valid is False
    assert missing_face_assessment.links_valid is False

    not_solved = copy.deepcopy(control)
    not_solved["shear"]["links"]["chord_candidates"][0][
        "off_not_evaluated"
    ] = "not_solved"
    not_solved_assessment = capacity.evaluate_combined_prerequisites(
        inp,
        not_solved,
    )
    assert not_solved_assessment.valid is False
    assert not_solved_assessment.links_valid is False

    for authority_override in (
        {"shear_axis": "y"},
        {"shear_tension": False},
    ):
        authority_assessment = capacity.evaluate_combined_prerequisites(
            _member_input(
                combined_on=True,
                shear_links=True,
                **authority_override,
        ...
        assert authority_assessment.valid is False
        assert authority_assessment.links_valid is False
    ...
    subdivided_assessment = capacity.evaluate_combined_prerequisites(
        inp,
        subdivided,
    )
    assert subdivided_assessment.valid is False
    assert subdivided_assessment.links_valid is False
    assert (
        "active longitudinal chord evidence is missing, malformed, or incomplete"
        in subdivided_assessment.reasons
    )
