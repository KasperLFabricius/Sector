...
boundary. Publication exports therefore run through one dedicated subprocess
entry point that cannot execute the Streamlit app. A real warm-up render is the
READY handshake, and a timeout terminates the worker and its descendant process
tree before a later request may start a clean one.
"""
...
_FROZEN_WORKER_FLAG = "--sector-publication-image-worker"
_MAX_PROTOCOL_MESSAGE_BYTES = 128 * 1024 * 1024
_WARMUP_FIGURE = {
...
WorkerLauncher = Callable[[], tuple[Any, Any]]
...
_END_OF_WRITES = object()


def _encode_protocol_message(message: tuple[Any, ...]) -> bytes:
    def encode_special(value: object) -> object:
        if isinstance(value, bytes):
            return {"__sector_png_bytes__": base64.b64encode(value).decode("ascii")}
        raise TypeError(
            f"unsupported image-export protocol value: {type(value).__name__}"
        )
    ...
    if len(encoded) > _MAX_PROTOCOL_MESSAGE_BYTES:
        raise ValueError("image-export protocol message is too large")
    return encoded + b"\n"


def _decode_protocol_message(payload: bytes) -> tuple[Any, ...]:
    def decode_special(value: dict[str, object]) -> object:
        if set(value) == {"__sector_png_bytes__"}:
            encoded = value["__sector_png_bytes__"]
            if not isinstance(encoded, str):
                raise ValueError("invalid image-export byte payload")
            try:
                return base64.b64decode(encoded, validate=True)
            except (binascii.Error, ValueError) as exc:
                raise ValueError("invalid image-export byte payload") from exc
        return value

    decoded = json.loads(payload.decode("ascii"), object_hook=decode_special)
    if not isinstance(decoded, list):
        raise ValueError("image-export protocol message must be an array")
    return tuple(decoded)


class _StreamConnection:
    """Tuple-message connection over a subprocess's dedicated stdio pipes."""
    ...
    def __init__(
        ...
        *,
        ...
        self._writes: queue.Queue[object] | None = (
            queue.Queue() if async_writes else None
        )
        self._pending: object = _NO_MESSAGE
    ...
    def _read_messages(self) -> None:
        try:
            while True:
                line = self._reader.readline(_MAX_PROTOCOL_MESSAGE_BYTES + 2)
                if not line:
                    self._messages.put(_END_OF_STREAM)
                    return
                if len(line) > _MAX_PROTOCOL_MESSAGE_BYTES + 1 or not line.endswith(
                    b"\n"
                ):
                    raise ValueError("invalid image-export protocol frame")
                self._messages.put(_decode_protocol_message(line[:-1]))
        except BaseException as exc:
            self._messages.put(_ProtocolFailure(exc))

    def _write_payload(self, payload: bytes) -> None:
        with self._write_lock:
            if self._closed:
                raise OSError("image-export protocol stream is closed")
    ...
    def _write_messages(self) -> None:
        ...
        try:
            while True:
                ...
                if payload is _END_OF_WRITES:
                    return
                if not isinstance(payload, bytes):
                    raise TypeError("invalid queued image-export protocol frame")
                self._write_payload(payload)
        except BaseException as exc:
            self._messages.put(_ProtocolFailure(exc))

    def poll(self, wait: float) -> bool:
        if self._pending is not _NO_MESSAGE:
            return True
        try:
            self._pending = self._messages.get(timeout=max(0.0, wait))
        except queue.Empty:
            return False
        return True
    ...
    def recv(self) -> tuple[Any, ...]:
        ...
        self._pending = _NO_MESSAGE
        if message is _END_OF_STREAM:
            raise EOFError("image-export worker closed its protocol stream")
        if isinstance(message, _ProtocolFailure):
            raise OSError("invalid image-export protocol stream") from message.error
        if not isinstance(message, tuple):
            raise OSError("invalid image-export protocol message")
        return message

    def send(self, message: tuple[Any, ...]) -> None:
        payload = _encode_protocol_message(message)
        if self._closed:
            raise OSError("image-export protocol stream is closed")
        if self._writes is not None:
    ...
    def close(self) -> None:
        ...
        # joined the worker, so both threads normally finish here.  In the
        # child, the blocked stdin reader is a daemon and may remain open until
        # interpreter teardown rather than deadlocking the worker's exit.
        streams = (
            (self._writer, self._writer_thread),
            (self._reader, self._reader_thread),
        )
        for stream, thread in streams:
...
class _SynchronousStreamConnection:
    """Worker-side protocol connection with no background I/O threads."""
    ...
    def recv(self) -> tuple[Any, ...]:
        line = self._reader.readline(_MAX_PROTOCOL_MESSAGE_BYTES + 2)
        if not line:
            raise EOFError("image-export parent closed its protocol stream")
        if len(line) > _MAX_PROTOCOL_MESSAGE_BYTES + 1 or not line.endswith(b"\n"):
            raise OSError("invalid image-export protocol frame")
        return _decode_protocol_message(line[:-1])

    def send(self, message: tuple[Any, ...]) -> None:
        if self._closed:
            raise OSError("image-export protocol stream is closed")
        self._writer.write(_encode_protocol_message(message))
        self._writer.flush()
...
class _SubprocessHandle:
    """Small process facade shared with the coordinator's test doubles."""
    ...
    def close(self) -> None:
        # Popen owns no additional closeable handle after its stdio streams have
        # been closed and the child has been reaped by join().
        return


def _worker_command() -> list[str]:
    """Return a worker entry point that cannot execute the Streamlit app."""

    if getattr(sys, "frozen", False):
        return [sys.executable, _FROZEN_WORKER_FLAG]
    worker = pathlib.Path(__file__).with_name("publication_image_export_worker.py")
...
def _launch_worker() -> tuple[_StreamConnection, _SubprocessHandle]:
    return _launch_worker_command(_worker_command())
...
class KaleidoExportCoordinator:
    ...
    def __init__(
        self,
        *,
        worker_launcher: WorkerLauncher = _launch_worker,
    ...
    def _start_locked(self, timeout: float) -> None:
        ...
        try:
            ...
            connection, process = self._worker_launcher()
