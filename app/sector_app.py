...
def _run_bridge_or_invalid(inp):
    ...
    return bridge_analysis.run_or_invalid(inp)
...
def bridge_view(inp, results):
    ...
    if failures:
        for failure in failures:
            st.error(
                "INVALID - bridge calculation | "
                f"{failure.get('family', 'bridge')} | "
                f"{failure.get('message', 'Unknown bridge failure')} "
                f"[{failure.get('code', 'INVALID_INPUT')}; "
