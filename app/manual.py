...
def manual_blocks() -> list:
    ...
    md("For bending with axial force, Sector derives the uncracked cracking action "
       ...
       "maximum intervals of 1, 0.1 and, where needed, 0.01 degrees. From the "
       "0.1-degree stage, the retained utilisation interval is "
       "$u_f \\pm \\max(2|u_f-u_c|,10^{-6})$, where $u_f$ and $u_c$ are successive "
       "fine and coarse results. A verdict is accepted only when successive stages "
       "agree and the whole interval lies on one side of the acceptance boundary "
       "$u=1+10^{-9}$; a case still crossing that boundary after the 0.01-degree "
       "stage is reported **Not assessed**. Pure tension uses:\n\n"
