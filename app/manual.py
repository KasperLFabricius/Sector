...
from publication_items import (
    published_manual_blocks,
    published_manual_parts,
)
...
def manual_published_item_parts():
    """Return the visible manual with exact Figure/Table identities attached."""

    parts = published_manual_parts(
...
def _manual_pdf_styles(
    ...
    _add("MPubCaption", fontSize=8, leading=10, textColor=colors.HexColor("#2c2c2a"),
         fontName=font, spaceBefore=2, spaceAfter=3, keepWithNext=1)
    return styles
...
def build_manual_pdf(buffer, figures=True):
    ...
    if figures:
        if _call_with_timeout(report.ensure_image_server,
            ...
            figures_hung = True
    for published in published_manual_blocks(
        manual_publication_blocks(manual_blocks())
    ):
